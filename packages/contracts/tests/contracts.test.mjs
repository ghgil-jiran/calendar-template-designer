...
import{CONTENT_PURPOSES,SCHOOL_DATA_SCHEMA,SURFACE_ROLES,TEMPLATE_PERSISTENCE_CONTRACT_VERSION,TEMPLATE_SAVE_KINDS,TEMPLATE_STATES,assertCalendarDocument,normalizeSchoolData,validateSchoolData,normalizeAcademicYear,normalizeCalendarEvent,validateCalendarEvent,normalizeImageFrameValue,mergeMonthlyStyle}from"../dist/index.js";
...
assert.ok(SURFACE_ROLES.includes("cover-back"));
assert.ok(SURFACE_ROLES.includes("back-cover-front"));
assert.ok(!SURFACE_ROLES.includes("annual-calendar"));
assert.ok(!SURFACE_ROLES.includes("school-symbols"));
assert.ok(CONTENT_PURPOSES.includes("annual-calendar"));
assert.ok(CONTENT_PURPOSES.includes("school-symbols"));
