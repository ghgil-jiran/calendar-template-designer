...
export class LayoutEngine {
 ...
 private fitText(o:RenderNode,d:RuntimeDiagnostic[],pageId:string){const text=o.payload as string,s=o.style,sizeKey=s.fontSizePt!==undefined?"fontSizePt":"fontSize",fs=Number(s[sizeKey]??12),physicalFs=sizeKey==="fontSizePt"?fs*25.4/72:fs,min=Number(s.minFontSize??Math.max(6,fs*.6)),lineHeight=Number(s.lineHeight??1.25),capacity=Math.max(1,(o.frame.width*o.frame.height)/Math.max(1,physicalFs*physicalFs*.5*lineHeight));s.overflow=s.overflow??"clip";s.verticalAlign=s.verticalAlign??"middle";s.whiteSpace=s.whiteSpace??"pre-wrap";if(text.length>capacity&&s.autoFit!==false){const ratio=Math.sqrt(capacity/text.length),next=Math.max(min,Math.floor(fs*ratio*10)/10);s[sizeKey]=next;d.push({severity:"info",code:"TEXT_AUTOFIT_APPLIED",message:`텍스트 자동 맞춤으로 글자 크기를 ${fs}에서 ${next}로 조정했습니다.`,pageId,objectId:o.id});if(next===min&&text.length>capacity*1.2)d.push({severity:"warning",code:"TEXT_OVERFLOW",message:"최소 글자 크기에서도 텍스트가 넘칠 수 있습니다.",pageId,objectId:o.id})}}
