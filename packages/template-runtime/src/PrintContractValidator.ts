...
const vectorTypes = new Set(["text", "shape", "vector", "line", "calendar", "calendar-grid", "year-calendar", "mini-calendar", "memo"]);
  ...
  validate(template:TemplateDocument):RuntimeDiagnostic[] {
    ...
    const inspect=(object:TemplateObject,pageId:string):void=>{
      ...
      if(!policy) diagnostics.push({severity:"error",code:"PRINT_OBJECT_POLICY_MISSING",message:"인쇄 개체 정책이 없습니다.",pageId,objectId:object.id});
        ...
        if(object.type==="text"&&policy.textMode!=="outline") diagnostics.push({severity:"error",code:"PRINT_TEXT_OUTLINE_REQUIRED",message:"인쇄 텍스트는 아웃라인이어야 합니다.",pageId,objectId:object.id});
