(function (root) {
  ...
  function applyDeskRepresentativePreset(project, options) {
    ...
    project.template.pageComposition = { type: 'desk-sequence', pageCount: 28, monthPairCount: 12 };
    ...
    if (coverBack) {
      ...
      project.book.elementsByPage[coverBack.id] = [{ id: 'page.yearly', type: 'year-calendar', role: 'year-calendar', x: 6, y: 8, width: 88, height: 84, zIndex: 1, startMonth: options.startMonth, columns: 4, style: {} }];
    }
    const symbolPage = project.book.pageInstances.find(page => page.role === 'back-cover-front');
    if (symbolPage) {
    ...
    project.book.pageInstances.forEach((page, index) => {
      ...
      if (page.role === 'back-cover-back') page.semanticPageRole = 'back-cover';
    });
    project.book.monthlyStyleOverrides = months.map((page, index) => ({ monthKey: page.monthKey, tokens: { primary: palette[index], accent: palette[index], calendarHeader: palette[index] } }));
  ...
  function createProject(options, dependencies) {
    ...
    if (isPoster) {
      ...
      for (let index = 1; index <= options.frontInsertCount; index += 1) specs.push(['front-insert', null, index]);
      months.forEach(month => specs.push(['monthly', month, null]));
      for (let index = 1; index <= options.rearInsertCount; index += 1) specs.push(['rear-insert', null, index]);
