(function(root){
 ...
 async function request(url,options={},timeoutMs=15000){
  ...
  try{
   ...
   if(!response.ok){const error=new Error(body?.error?.message||body?.details?.message||body?.message||'AI 디자인 연결 요청에 실패했습니다.');error.status=response.status;error.code=body?.error?.code||body?.error||null;error.reason=body?.details?.reason||null;error.retryAfterSeconds=Number(body?.details?.retryAfterSeconds)||0;throw error}
   return body;
