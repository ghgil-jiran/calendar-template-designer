(function(root){
 ...
 function legacyObject(element,width,height,index,project){
  ...
  return {id:String(element.id||`legacy.object.${index}`),sourceObjectId:String(element.id||`legacy.object.${index}`),type:objectType(element.type),role:String(element.role||''),frame:{x:pct(element.x,width),y:pct(element.y,height),width:pct(element.width==null?10:element.width,width),height:pct(element.height==null?10:element.height,height)},binding,value,runtimeWidget:{rows:element.rows,weekStart:element.weekStart,showWeekday:element.showWeekday,showDate:element.showDate,memoLayout:element.memoLayout,title:element.title,lineCount:element.lineCount,itemCount:element.itemCount,weekCount:element.weekCount,showMemo:element.showMemo,columns:element.columns,startMonth:element.startMonth,monthCount:element.monthCount,monthLabelStyle:element.monthLabelStyle,yearlyColumns:element.yearlyColumns,linesPerMonth:element.linesPerMonth,baseYear:element.baseYear,yearlyLayoutType:element.yearlyLayoutType,yearlyGroupSize:element.yearlyGroupSize,yearlyContainerStyle:element.yearlyContainerStyle,scheduleLayoutType:element.scheduleLayoutType,dateBased:element.dateBased,equalCells:element.equalCells,monthSource:element.monthSource,showEmptyMessage:element.showEmptyMessage,emptyStatePresentation:element.emptyStatePresentation,layoutType:element.layoutType},style:{...(element.style||{}),legacyRole:element.role},visible:element.visible!==false,zIndex:Number(element.zIndex==null?index:element.zIndex)}
 }
 function create(options={}){
  ...
  function adapt(project,pageInstances=project.book.pageInstances||[],datasetOverride){
   ...
   const pages=pageInstances.map((page,index)=>{
    const sourcePageId=page.sourcePageId||page.id,master=project.template.masterElements?.[page.masterId]||[],local=project.book.elementsByPage?.[sourcePageId]||[],shadowed=new Set(local.map(element=>element.shadowOfMasterElementId).filter(Boolean)),hideInheritedDecoration=page.aiDesignBase?.mode==='neutral',replaceMonthBackComposition=page.aiMonthBackComposition?.mode==='generated-layout',isMonthBackComposition=element=>element?.role==='ai-month-back-component'||Boolean(element?.aiDesignComponent)||['image-frame','mini-calendar','mini-calendar-prev','mini-calendar-next','month-date-strip','memo'].includes(objectType(element?.type))||['monthly-goal','monthly-todo','weekly-planner'].includes(element?.role),visibleMaster=master.filter(element=>!shadowed.has(element.id)).filter(element=>!hideInheritedDecoration||!(element?.role==='background-decoration'||element?.role==='ai-design-background'||Boolean(element?.backgroundPresetId))).filter(element=>!replaceMonthBackComposition||!isMonthBackComposition(element)),objects=[...visibleMaster,...local].map((element,objectIndex)=>legacyObject(element,size.width,size.height,objectIndex,project));
