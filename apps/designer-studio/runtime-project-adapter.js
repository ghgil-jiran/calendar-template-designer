(function(root){
 ...
 function legacyObject(element,width,height,index,project){
  ...
  return {id:String(element.id||`legacy.object.${index}`),sourceObjectId:String(element.id||`legacy.object.${index}`),type:objectType(element.type),role:String(element.role||''),frame:{x:pct(element.x,width),y:pct(element.y,height),width:pct(element.width==null?10:element.width,width),height:pct(element.height==null?10:element.height,height)},binding,value,runtimeWidget:{rows:element.rows,weekStart:element.weekStart,showWeekday:element.showWeekday,showDate:element.showDate,memoLayout:element.memoLayout,title:element.title,lineCount:element.lineCount,itemCount:element.itemCount,weekCount:element.weekCount,showMemo:element.showMemo},style:{...(element.style||{}),legacyRole:element.role},visible:element.visible!==false,zIndex:Number(element.zIndex==null?index:element.zIndex)}
 }
