...
function populateExportSettings(){ensureTemplateResources();const e=project.template.resources.exportSettings,type=project?.template?.calendarTypeSnapshot?.definition||{},finished=type.finishedSize||project?.productType?.pageSize||{},production=type.productionSize||{},bleed=Math.max(0,(Number(production.width||finished.width)-Number(finished.width||0))/2)||3;el("exportFormat").value="pdf";el("exportDpi").value="300";el("exportBleed").value=bleed;el("exportCropMarks").value="true";el("exportColorMode").value="cmyk";el("exportPageRange").value=e.pageRange||"all";el("exportImageQuality").value=e.imageQuality||"high";el("exportGuides").value=String(e.guides===true);updateExportSummary()}
function updateExportSummary(){const box=el("exportSettingsSummary");if(!box)return;const type=project?.template?.calendarTypeSnapshot?.definition||{},trim=type.finishedSize||project?.productType?.pageSize||{},production=type.productionSize||{width:Number(trim.width||0)+6,height:Number(trim.height||0)+6},dpi=300,px=value=>Math.ceil(Number(value||0)/25.4*dpi),generatedWidth=1536,generatedHeight=1024,effectiveDpi=Math.floor(Math.min(generatedWidth/Number(production.width||1),generatedHeight/Number(production.height||1))*25.4);box.innerHTML=`<strong>desk-calendar-print.v1</strong><br>완성 ${trim.width||0}×${trim.height||0}mm · 제작 ${production.width||0}×${production.height||0}mm · PDF/X-4 · CMYK · Japan Color 2011 Coated · K100<br>AI 목표 ${px(production.width)}×${px(production.height)}px (${dpi} DPI) · 1차 생성 1536×1024px (약 ${effectiveDpi} DPI) · 인쇄 전 업스케일 필요<br>검토 범위 ${el("exportPageRange").value==='all'?'전체 페이지':'현재 페이지'} · 검토용 이미지 품질 ${el("exportImageQuality").value}`}
