...
function renderSemanticObject(item){
 ...
 const title=item.showTitle===false?"":`<strong style="font-size:${item.style?.titleSize||18}px">${item.titleOverride||semanticRoleLabel(role)}</strong>`;
 ...
 return `<div class="semantic-object ${layout==="image-left"?"semantic-layout-row":""}"><div class="semantic-media">${image}</div>${item.showCaption===true||item.showTitle!==false?`<div class="semantic-copy" style="text-align:${item.style?.textAlign||"center"}">${title}${item.showCaption===false?"":`<p style="font-size:${item.style?.descriptionSize||11}px">${d.description||""}</p>`}</div>`:""}</div>`
}
...
function bindInspector(){
 ...
 bind("applySemanticTemplate",()=>change(()=>{
  ...
  i.showTitle=el("semanticShowTitle")?.checked!==false;i.titleOverride=el("semanticTitleOverride")?.value.trim()||semanticRoleLabel(i.role);i.style.titleSize=Number(el("semanticTitleSize").value);
