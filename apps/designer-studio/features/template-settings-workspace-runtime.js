...
(()=>{
 ...
 function txGet(store,key){return assetDatabase.get(store,key)}
 ...
 async function originalDataUrl(assetId){const record=await txGet('assets',assetId);if(!record?.blob)throw new Error(`이미지 원본을 찾지 못했습니다: ${assetId}`);return blobToDataURL(record.blob)}
 window.ACDLAssetStore={storeImage,originalDataUrl,formatBytes,openDB};
