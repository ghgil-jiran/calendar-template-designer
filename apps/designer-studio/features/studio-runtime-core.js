...
function updateSetupFields(){const type=el("setupType").value;document.querySelectorAll(".setup-field[data-types]").forEach(node=>node.classList.toggle("hidden-by-type",!node.dataset.types.split(/\s+/).includes(type)));const nativeOption=el('setupTemplate')?.querySelector('[value="native-desk-academic-full"]');if(nativeOption){nativeOption.disabled=type!=='desk';if(nativeOption.selected&&nativeOption.disabled)el('setupTemplate').value='school-basic'}el("setupTypeNote").textContent=SETUP_TYPE_NOTES[type]||"";renderSizeOptions()}
...
function makeProject(opts){
 const dependencies={sizePresets:SIZE_PRESETS,buildMonths:monthSequence};
 return opts.template==='native-desk-academic-full'?window.ACDLNativePrintFullTemplate.create(opts,dependencies):window.ACDLProjectDocument.createProject(opts,dependencies)
}
...
function roleLabel(p){const map={"cover-front":"표지","cover-back":"표지 안쪽면","front-insert-front":"앞 간지 앞면","front-insert-back":"앞 간지 뒷면","rear-insert-front":"뒤 간지 앞면","rear-insert-back":"뒤 간지 뒷면","monthly-front":"월력 앞면","monthly-back":"월력 뒷면","back-cover-front":"뒷표지 안쪽면","back-cover-back":"뒷표지","poster-annual":"연간 벽보"},semantic={"yearly-calendar":"표지 안쪽면 · 연력","cover-continuation":"표지 디자인 연장","school-symbols":"학교 상징","back-cover-information":"뒷표지 안쪽면 · 학교 정보","back-cover-continuation":"뒷표지 안쪽면 · 표지 디자인 연장"};const idx=p.insertIndex?` ${p.insertIndex}`:"";return p.calendarMonth?`${p.calendarYear}.${String(p.calendarMonth).padStart(2,"0")} · ${map[p.role]}`:`${semantic[p.semanticPageRole]||map[p.role]||p.role}${idx}`}
...
el("saveExportSettingsBtn").addEventListener("click",()=>{snapshot();ensureTemplateResources();const definition=project?.template?.calendarTypeSnapshot?.definition||{},finished=definition.finishedSize||project?.productType?.pageSize||{},production=definition.productionSize||{},bleed=Math.max(0,(Number(production.width||finished.width)-Number(finished.width||0))/2)||3;project.template.resources.exportSettings={format:"pdf",dpi:300,bleed,cropMarks:true,colorMode:"cmyk",pageRange:el("exportPageRange").value,imageQuality:el("exportImageQuality").value,guides:el("exportGuides").value==='true',profileId:"desk-calendar-print.v1",pdfStandard:"PDF/X-4",outputIntent:"Japan Color 2011 Coated",blackPolicy:"K100"};markDirty();render();populateExportSettings();showEditorToast("검토 옵션을 저장했습니다. 인쇄 기술 기준은 제품 규격에서 자동 적용됩니다.")});
el("prepareAIDesignMockBtn").addEventListener("click",showCompletedAIDesignResults);
el("aiDesignProposalSection").addEventListener("click",event=>{const regenerateDesignButton=event.target.closest("[data-regenerate-ai-design]");if(regenerateDesignButton){regenerateRepresentativeAIDesign();return}if(!aiDesignMockSession)return;const selectButton=event.target.closest("[data-select-ai-design]");if(selectButton){aiDesignMockSession=window.ACDLAIDesignSettings.selectVariant(aiDesignMockSession,selectButton.dataset.selectAiDesign);aiMonthlyExpansionState="idle";renderAIDesignMockSession();showEditorToast("대표 디자인을 선택했습니다. 페이지 설정의 월력 뒷면 배치를 적용해 월력 전체 생성을 진행할 수 있습니다.");return}const regenerateButton=event.target.closest("[data-regenerate-ai-page]");if(regenerateButton&&aiDesignMockSession.selectedVariantId){aiDesignMockSession=window.ACDLAIDesignSettings.regeneratePage(aiDesignMockSession,aiDesignMockSession.selectedVariantId,regenerateButton.dataset.regenerateAiPage);renderAIDesignMockSession();showEditorToast("선택한 페이지 역할만 모의 재생성했습니다. 실제 AI 이미지는 아직 만들지 않습니다.")}});
...
async function representativeInlineUrl(value){if(!value||value.startsWith('data:')||value.startsWith('#'))return value;try{const response=await fetch(value);if(!response.ok)throw new Error(String(response.status));return await representativeBlobDataUrl(await response.blob())}catch(error){throw Object.assign(new Error('첫 페이지 이미지 자산을 읽지 못해 대표 이미지를 저장할 수 없습니다.'),{code:'THUMBNAIL_SOURCE_IMAGE_FAILED',cause:error})}}
...
async function representativeCoverSvg(sourceProject=project,pageIndex=0){
 ...
 try{
  ...
  await Promise.all([...source.querySelectorAll('img')].map(async image=>{if(!image.complete||!image.naturalWidth)await image.decode();if(!image.naturalWidth)throw Object.assign(new Error('첫 페이지에 표시할 이미지가 완전히 로드되지 않았습니다.'),{code:'THUMBNAIL_SOURCE_IMAGE_FAILED'})}));
  ...
  for(let index=0;index<Math.min(sourceNodes.length,cloneNodes.length);index++){
   const sourceNode=sourceNodes[index],cloneNode=cloneNodes[index],computed=getComputedStyle(sourceNode);cloneNode.setAttribute('style',[...computed].map(name=>`${name}:${computed.getPropertyValue(name)};`).join(''));
   if(sourceNode instanceof HTMLImageElement){const url=sourceNode.currentSrc||sourceNode.src;cloneNode.setAttribute('src',await representativeInlineUrl(url))}
   const background=computed.backgroundImage,match=background&&background!=='none'?background.match(/^url\(["']?(.*?)["']?\)$/):null;if(match){const data=await representativeInlineUrl(match[1]);cloneNode.style.backgroundImage=`url(${JSON.stringify(data)})`}
  }
  clone.querySelectorAll('.editor-only,.non-output,.s2-selection-toolbar,.s2-key-hint').forEach(node=>node.remove());clone.querySelectorAll('.selected').forEach(node=>node.classList.remove('selected'));
...
el("libraryBtn")?.addEventListener("click",()=>{if(appMode!=="designer")return;el("templateLibraryModal").classList.remove("hidden");renderTemplateLibrary();refreshRemoteTemplateLibrary().then(refreshed=>refreshed?window.ACDLTemplatePublishing?.synchronizeCatalog?.():null).then(result=>result?refreshRemoteTemplateLibrary():null).catch(error=>console.warn("사용자 서비스 템플릿 목록 동기화 실패",error))});el("closeTemplateLibraryBtn")?.addEventListener("click",closeTemplateLibrary);el("newLibraryTemplateBtn")?.addEventListener("click",()=>{clearNewTemplateBase();openDesignerStudio({mode:"designer",source:"library"})});
...
el("confirmTemplateSaveBtn")?.addEventListener("click",async ()=>{
 ...
 const previousSaveState=project?.template?.metadata?.state,projectBeforeSave=project?window.ACDLPersistenceProject.clone(project):null;
 try{
  const dialog=el("templateSaveDialog");
  if(dialog.dataset.mode==="settings"){await window.ACDLTemplateLibrarySettings.save(dialog.dataset.recordId,{name:el("saveTemplateName").value.trim(),description:el("saveTemplateDescription").value.trim(),edition:Number(el("saveTemplateEdition").value),state:el("saveTemplateState").value,isStandard:el("saveTemplateStandard").checked});dialog.classList.add("hidden");window.ACDLTemplateSaveJob?.complete('템플릿 설정과 사용자 서비스 목록 동기화가 완료되었습니다.');return}
  ...
  if(remote?.isRemote?.())try{const result=await remote.save({templateId:project.template.remoteId||null,stableKey,name,description,edition,state,isStandard,productType:project.productType?.category||project.settings?.type||"desk",templateKey:project.template?.preset||project.settings?.template||"school-basic",saveKind:state==="published"?"publish":"manual",saveNote:`${name} 저장`,schemaVersion:"2.0",projectData:projectCopy},{onProgress:window.ACDLTemplateSaveProgress,verifyThumbnail:true});savedId=result.template.id;remoteVersion=result.version.versionNumber;remoteSaved=true;const persistedThumbnail=result.version?.projectData?.template?.thumbnail;if(!persistedThumbnail?.dataUrl||state==='published'&&persistedThumbnail.packagePreview?.sha256!==publicationResult?.representativePreview?.sha256)throw Object.assign(new Error('서버에 저장된 대표 이미지가 생성한 이미지와 일치하지 않습니다.'),{code:'REPRESENTATIVE_PREVIEW_MISMATCH'});projectCopy.template.thumbnail=window.ACDLPersistenceProject.clone(persistedThumbnail);project.template.thumbnail=window.ACDLPersistenceProject.clone(persistedThumbnail);project.template.id=savedId;project.template.remoteId=savedId;project.template.remoteStableKey=result.template.stableKey;project.template.remoteVersionNumber=remoteVersion;await saveTemplateProjectData(savedId,window.ACDLPersistenceProject.clone(project));if(state==="published"){window.ACDLTemplateSaveProgress?.({phase:'publishing',stage:'cleanup',detail:'사용자 서비스의 활성 템플릿 목록을 동기화하고 있습니다.'});await window.ACDLTemplatePublishing.synchronizeCatalog()}}catch(error){remoteError=error;console.warn("원격 템플릿 저장 실패",error);throw error}
  project.template.librarySource="local";
  const list=v22Library(),record={id:savedId,remoteId:remoteSaved?savedId:undefined,stableKey,name,description,edition,state,isStandard,libraryScope:project.template.libraryScope,type:project.productType?.category||project.settings?.type||"desk",template:project.template?.preset||project.settings?.template||"school-basic",packageVersion:project.template?.publishing?.lastReviewPackage?.version||project.template?.package?.version,lastReviewPackage:project.template?.publishing?.lastReviewPackage,packageBase:project.template?.package?.base,derivedFromPackage:project.template?.derivedFromPackage||undefined,source:"local",thumbnail:project.template.thumbnail,version:remoteVersion||1,updatedAt:new Date().toISOString(),storage:remoteSaved?"supabase":"indexeddb"};
  ...
  if(publicationResult)window.ACDLTemplatePublishing.completePublication?.(publicationResult.templateId,publicationResult.version);window.ACDLTemplateSaveJob?.complete(`${name} · ${remoteSaved?`편집 이력 v${remoteVersion}`:'브라우저 저장'} 저장이 완료되었습니다.`);showEditorToast(`${name} ${edition} Edition ${remoteSaved?`원격 저장 완료 · v${remoteVersion}`:"브라우저 저장 완료"}`);
 }catch(err){
  if(projectBeforeSave)project=projectBeforeSave;else if(project?.template?.metadata&&previousSaveState!==undefined)project.template.metadata.state=previousSaveState;
  feedback.className="save-feedback error";feedback.textContent=err?.code==="PUBLISH_CANCELLED"?"게시를 취소했습니다.":String(err?.code||"").startsWith("THUMBNAIL_")&&err?.code!=="THUMBNAIL_SOURCE_IMAGE_FAILED"&&err?.code!=="THUMBNAIL_RENDER_EMPTY"?`버전 저장 후 대표 이미지 검증 실패: ${err.message} 라이브러리의 최신 버전을 확인해 주세요.`:`저장 실패${err?.stage?` (${err.stage})`:""}: ${err?.message||"브라우저 데이터베이스 저장 오류"}`;window.ACDLTemplateSaveJob?.fail(err,()=>el("confirmTemplateSaveBtn")?.click());
 }
});
...
(function(){
  ...
  window.applyAllBoundAssets=applyAllBoundAssets;
...
renderFreeElements=function(pageNode){
 ...
 allVisibleElements().forEach(view=>{
  const box=document.createElement("div");box.className=`free-element ${view._scope==="master"?"master-element":""} ${view.id===selectedElementId&&view._scope===selectedElementScope?"active":""}`;box.dataset.elementId=view.id;box.dataset.scope=view._scope;box.dataset.elementType=view.type;box.dataset.elementRole=view.role||"";box.dataset.containerStyle=view.style?.containerStyle||"none";box.dataset.sectionDivider=view.style?.sectionDivider||"none";box.style.left=view.x+"%";box.style.top=view.y+"%";box.style.width=view.width+"%";box.style.height=view.height+"%";box.style.zIndex=view.zIndex||1;
...
function permissionHTML(item){
 ...
 const monthlyRole=selectedPage()?.role,monthlyScope=["monthly-front","monthly-back"].includes(monthlyRole)&&matchingMonthlyObjectsFor(item).length>1;
 ...
 ${monthlyScope?`<label class="permission-compact-option"><input type="checkbox" id="applyMonthlyPermissions" ${item.shadowOfMasterElementId?"":"checked"}><span><strong>같은 월력 ${monthlyRole==="monthly-front"?"앞면":"뒷면"} 전체 적용</strong><small>같은 역할의 개체에 권한을 함께 저장합니다. 이 월만 적용하려면 해제하세요.</small></span></label>`:""}
...
function printImageApprovalMarkup(item){if(project.template?.nativePrintAuthoring?.enabled!==true)return"";const source=item.printSource;if(!source)return `<div class="hint">인쇄 원본 미등록 · 화면 미리보기만 사용됩니다.</div>`;if(!source.eligible)return `<div class="hint">인쇄 사용 불가 · 4채널 CMYK JPEG 원본이 필요합니다.</div>`;const minimum=Number(project.template.nativePrintAuthoring.contract?.minimumImageDpi||300),placement=window.ACDLNativePrintAuthoring.inspectPrintImagePlacement(item,project.productType.pageSize,minimum),size=`${source.width}×${source.height}px`,dpi=`유효 ${placement.effectiveDpi}DPI / 최소 ${minimum}DPI`;if(placement.status!=='passed')return `<div class="hint">CMYK JPEG 기술 검사 통과 · ${size}</div><div class="hint">인쇄 승격 차단 · ${dpi}. 프레임 크기나 확대 배율을 조정하세요.</div>${source.approved&&project.mode!=='calendar-workspace'?`<button id="revokePrintImageApproval" class="action secondary" type="button">인쇄 승인 해제</button>`:''}`;if(source.approved)return `<div class="hint">인쇄 승인됨 · ${size} · ${dpi} · ${source.outputConditionIdentifier}</div>${project.mode==='calendar-workspace'?'':`<button id="revokePrintImageApproval" class="action secondary" type="button">인쇄 승인 해제</button>`}`;if(project.mode==='calendar-workspace')return `<div class="hint">CMYK JPEG 기술 검사 통과 · ${size} · ${dpi} · 관리자 출력조건 승인 대기</div>`;return `<div class="hint">CMYK JPEG 기술 검사 통과 · ${size} · ${dpi}</div><div class="hint">이 원본이 ${source.proposedOutputConditionIdentifier} 기준으로 제작되었음을 확인한 경우에만 승인하세요.</div><button id="approvePrintImageOutputCondition" class="action secondary" type="button">출력조건 확인 및 인쇄 승인</button>`}
let nativePrintReadinessExpanded=false;
const nativePrintReadinessLabels={PRINT_INTENT_MISSING:'인쇄 의미 미지정',RUNTIME_EXPANSION_REQUIRED:'Runtime 기본 개체 확장 필요',OBJECT_TYPE_UNSUPPORTED:'지원하지 않는 개체',PRINT_STRUCTURE_UNSUPPORTED:'인쇄 구조 미지원',PRINT_CMYK_COLOR_UNRESOLVED:'CMYK 색상 미지정',PRINT_FONT_ASSET_MISSING:'인쇄 폰트 누락',PRINT_FONT_PACKAGE_PENDING:'폰트 Package 승격 대기',PRINT_IMAGE_ORIGINAL_MISSING:'인쇄 이미지 원본 없음',PRINT_IMAGE_NOT_CMYK_JPEG:'CMYK JPEG 원본 필요',PRINT_IMAGE_DPI_BELOW_MINIMUM:'이미지 유효 DPI 부족',PRINT_IMAGE_OUTPUT_CONDITION_UNAPPROVED:'이미지 출력조건 승인 필요',PRINT_IMAGE_PACKAGE_PENDING:'이미지 Package 승격 대기',PRINT_IMAGE_ASSET_UNAPPROVED:'이미지 Package 자산 미승인'};
function renderNativePrintReadinessPanel(){const host=el('nativePrintReadinessPanel');if(!host)return;if(project?.template?.nativePrintAuthoring?.enabled!==true){host.className='native-print-readiness hidden';host.innerHTML='';return}const report=window.ACDLNativePrintPackageCompiler.inspectProjectReadiness(project),state=report.status,label=state==='ready'?'인쇄 승격 준비 완료':state==='pending'?'Package 승격 대기':'인쇄 승격 차단',items=report.items.filter(item=>item.status!=='ready');host.className=`native-print-readiness ${state}`;host.innerHTML=`<button id="toggleNativePrintReadiness" type="button" aria-expanded="${nativePrintReadinessExpanded}"><span>${label}</span><span>${nativePrintReadinessExpanded?'접기':'상세'}</span></button><div class="summary"><span>전체 ${report.counts.objects}</span><span>준비 ${report.counts.ready}</span><span>대기 ${report.counts.pending}</span><span>차단 ${report.counts.blocked}</span></div>${nativePrintReadinessExpanded?`<div class="issues">${items.length?items.slice(0,20).map(item=>`<button type="button" class="${item.status}" data-native-print-object="${v21Escape(item.objectId)}" data-native-print-scope="${item.scope}" data-native-print-container="${v21Escape(item.containerId)}"><span>${v21Escape(item.role||item.objectType||item.objectId)}</span><b>${item.status==='pending'?'대기':'차단'}</b><small>${v21Escape(nativePrintReadinessLabels[item.code]||item.code)}${item.effectiveDpi!==undefined?` · ${item.effectiveDpi}DPI`:''}</small></button>`).join(''):`<small>모든 개체가 준비되었습니다.</small>`}${items.length>20?`<small>외 ${items.length-20}개 항목</small>`:''}</div>`:''}`}
const nativePrintImageRenderInspector=renderInspector;
renderInspector=function(){const result=nativePrintImageRenderInspector.apply(this,arguments),item=sourceElement();renderNativePrintReadinessPanel();if(item?.type==='image-frame'){const button=el('replaceFrameImageBtn');button?.insertAdjacentHTML('afterend',printImageApprovalMarkup(item))}return result};
...
document.addEventListener("click",e=>{const action=e.target.closest('button')?.id;if(!['approvePrintImageOutputCondition','revokePrintImageApproval'].includes(action)||project.mode==='calendar-workspace')return;const item=sourceElement(),source=item?.printSource;if(!source?.eligible)return;const minimum=Number(project.template.nativePrintAuthoring.contract?.minimumImageDpi||300),placement=window.ACDLNativePrintAuthoring.inspectPrintImagePlacement(item,project.productType.pageSize,minimum);if(action==='approvePrintImageOutputCondition'&&placement.status!=='passed'){showEditorToast(`유효 해상도 ${placement.effectiveDpi}DPI로 인쇄 승격할 수 없습니다.`);return}snapshot();if(action==='approvePrintImageOutputCondition'){window.ACDLNativePrintAuthoring.approvePrintImageSource(source,project.template.nativePrintAuthoring.contract.outputIntent,placement);showEditorToast('CMYK 인쇄 원본·출력조건·유효 DPI를 승인했습니다.')}else{window.ACDLNativePrintAuthoring.revokePrintImageSource(source);showEditorToast('인쇄 원본 승인을 해제했습니다.')}markDirty();render()});
document.addEventListener('click',e=>{const toggle=e.target.closest('#toggleNativePrintReadiness');if(toggle){nativePrintReadinessExpanded=!nativePrintReadinessExpanded;renderNativePrintReadinessPanel();return}const target=e.target.closest('[data-native-print-object]');if(!target)return;const scope=target.dataset.nativePrintScope,containerId=target.dataset.nativePrintContainer;if(scope==='page')selectedPageId=containerId;else{const page=project.book.pageInstances.find(item=>item.masterId===containerId);if(page)selectedPageId=page.id}selectedElementId=target.dataset.nativePrintObject;selectedElementScope=scope;render()});
el("frameImageInput").addEventListener("change",async e=>{const file=e.target.files?.[0],targetId=pendingImageElementId||selectedElementId,targetScope=pendingImageElementScope||selectedElementScope;e.target.value="";if(!file||!targetId)return;const arr=targetScope==="master"?masterElements():pageElements(),item=arr.find(x=>x.id===targetId);if(item?.type!=="image-frame"){pendingImageElementId=null;pendingImageElementScope=null;return}try{showEditorToast("이미지를 최적화하고 있습니다…");const stored=await window.ACDLAssetStore.storeImage(file,{previewMax:900,previewQuality:.78}),inspection=project.template?.nativePrintAuthoring?.enabled===true?await window.ACDLNativePrintAuthoring.inspectPrintImageFile(file,project.template.nativePrintAuthoring.contract?.outputIntent):null;snapshot();item.image||={};item.image.assetId=stored.assetId;item.image.src=stored.preview;item.image.originalName=file.name;item.image.originalBytes=file.size;item.image.fit||="cover";item.image.scale||=1;item.image.offsetX??=0;item.image.offsetY??=0;if(inspection)item.printSource={assetId:stored.assetId,...inspection};else delete item.printSource;selectedElementId=item.id;selectedElementScope=targetScope;markDirty();render();showEditorToast(inspection?.eligible?`CMYK JPEG 원본을 분리 저장했습니다. 출력조건 승인이 필요합니다. ${inspection.width}×${inspection.height}px`:`화면 미리보기를 저장했습니다. 인쇄 원본 후보는 4채널 CMYK JPEG여야 합니다.`)}catch(err){showEditorToast(err?.message||"이미지를 읽지 못했습니다.")}finally{pendingImageElementId=null;pendingImageElementScope=null;window.__acdlUpdateMemoryMonitor?.()}});
