...
el("libraryBtn")?.addEventListener("click",()=>{if(appMode!=="designer")return;el("templateLibraryModal").classList.remove("hidden");renderTemplateLibrary();refreshRemoteTemplateLibrary().then(refreshed=>refreshed?window.ACDLTemplatePublishing?.synchronizeCatalog?.():null).then(result=>result?refreshRemoteTemplateLibrary():null).catch(error=>console.warn("사용자 서비스 템플릿 목록 동기화 실패",error))});el("closeTemplateLibraryBtn")?.addEventListener("click",closeTemplateLibrary);el("newLibraryTemplateBtn")?.addEventListener("click",()=>{clearNewTemplateBase();el("templateLibraryModal").classList.add("hidden");enterDesigner()});
