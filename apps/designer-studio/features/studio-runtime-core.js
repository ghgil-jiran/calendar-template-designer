...
el("confirmTemplateSaveBtn")?.addEventListener("click",async ()=>{
 ...
 try{
  ...
  if(dialog.dataset.mode==="settings"){const settingsState=['draft','ready','archived'].includes(el("saveTemplateState").value)?el("saveTemplateState").value:'draft';await window.ACDLTemplateLibrarySettings.save(dialog.dataset.recordId,{name:el("saveTemplateName").value.trim(),description:el("saveTemplateDescription").value.trim(),edition:Number(el("saveTemplateEdition").value),state:settingsState,isStandard:false});dialog.classList.add("hidden");return}
  normalizeElementData();ensureV22Metadata();
  const name=el("saveTemplateName").value.trim()||"이름 없는 템플릿",description=el("saveTemplateDescription").value.trim(),edition=Number(el("saveTemplateEdition").value)||2027,state=['draft','ready','archived'].includes(el("saveTemplateState").value)?el("saveTemplateState").value:'draft',isStandard=false;
  ...
  window.ACDLReturnToLibraryOnSaveCancel=false;stable();el("templateSaveDialog").classList.add("hidden");if(state==="ready"||state==="archived"){el("templateLibraryModal").classList.remove("hidden");renderTemplateLibrary("all")}else render();
