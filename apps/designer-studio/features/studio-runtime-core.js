...
function roleLabel(p){const map={"cover-front":"표지 앞면","cover-back":"표지 뒷면","front-insert-front":"앞 간지 앞면","front-insert-back":"앞 간지 뒷면","rear-insert-front":"뒤 간지 앞면","rear-insert-back":"뒤 간지 뒷면","monthly-front":"월력 앞면","monthly-back":"월력 뒷면","back-cover-front":"맨 뒷표지 앞면","back-cover-back":"맨 뒷표지 뒷면","poster-annual":"연간 벽보"},semantic={"yearly-calendar":"표지 뒷면 · 연력","cover-continuation":"표지 디자인 연장","school-symbols":"학교 상징","back-cover-information":"뒷표지 앞면 · 학교 정보","back-cover-continuation":"뒷표지 앞면 · 표지 디자인 연장"};const idx=p.insertIndex?` ${p.insertIndex}`:"";return p.calendarMonth?`${p.calendarYear}.${String(p.calendarMonth).padStart(2,"0")} · ${map[p.role]}`:`${semantic[p.semanticPageRole]||map[p.role]||p.role}${idx}`}
