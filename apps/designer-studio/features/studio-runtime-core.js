...
el("confirmTemplateSaveBtn")?.addEventListener("click",async ()=>{
 ...
 try{
  ...
  feedback.className="save-feedback error";feedback.textContent=err?.code==="PUBLISH_CANCELLED"?"게시를 취소했습니다.":`저장 실패${err?.stage?` (${err.stage})`:""}: ${err?.message||"브라우저 데이터베이스 저장 오류"}`;
 }
});
