(()=>{
 ...
 const isReviewableSource=value=>typeof value==="string"&&(/^(data:image\/|blob:|https?:\/\/)/.test(value));
 ...
 function collectAssets(){
  const resources=(project?.template?.resources?.aiDesignAssets||[]).filter(asset=>asset?.source?.type==="live-ai-generation"&&isReviewableSource(asset.src));
 ...
 function expectedTargets(){
  const pages=project?.book?.pageInstances||[],roleForPage=window.ACDLDesignSetExpansion?.generatedRole,monthForPage=window.ACDLDesignSetExpansion?.monthKey;
  if(typeof roleForPage!=="function")return [];
  return pages.map(page=>({pageId:page.id,label:pageLabel(page),generatedRole:roleForPage(page),monthKey:typeof monthForPage==="function"?monthForPage(page):page.monthKey||""})).filter(target=>target.generatedRole);
 }
 function reviewStats(assets){
  const targets=expectedTargets(),assignedPageIds=new Set(assets.flatMap(asset=>asset.assignments||[]).map(item=>item.pageId)),missingTargets=targets.filter(target=>!assignedPageIds.has(target.pageId));
  ...
  return {expected:targets.length||assets.length,actual:assets.length,assigned:assignedPageIds.size,missingTargets,verified,high};
 }
 function metaRows(asset){
  ...
  return [
   ...
   ["모델",generation.model||evidence.model||String(generation.id||"").split("/")[0]||"확인 불가"],
 ...
 function createSummarySheet(assets){
  ...
  if(stats.missingTargets.length)sheet.append(text("p",`누락 대상: ${stats.missingTargets.map(target=>target.label).join(", ")}`,"ai-review-missing"));
 ...
 window.ACDLAIImageReviewPdf=Object.freeze({collect:collectAssets,expected:expectedTargets,stats:reviewStats,export:exportAiImageReviewPdf});
})();
