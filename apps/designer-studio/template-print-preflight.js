(()=>{
 ...
 function analyze(project,{templateId=null,version=null,runtimeDocument=undefined,renderParity=undefined,printOutput=undefined}={}){
  ...
  const approval=printOutput?.approval||{automated:{status:printOutput?.artifactVerified?'passed':'pending',label:'자동 인쇄 Preflight'},external:{status:'not_run',label:'Acrobat 외부 Preflight',required:true},physical:{status:'not_run',label:'실물 인쇄 승인',required:true},finalApproved:false,artifactClass:printOutput?.artifact?'legacy-converted':'not-generated'},finalReviewPending=printOutput!==undefined&&!approval.finalApproved&&!errors;
  ...
  const finalApprovalGate=mergeGate('외부·실물 승인',[4,5]),baseGates=[mergeGate('생성 준비',[0,1]),printDocumentGate,corePdfGate,aiImageGate,finalApprovalGate];
