(()=>{
 ...
 function analyze(project,{templateId=null,version=null,runtimeDocument=undefined,renderParity=undefined,printOutput=undefined}={}){
  ...
  const aiImageGate={name:'AI 생성 이미지 검사',status:aiInspection.status==='blocked'?'blocked':aiInspection.status==='passed'?'passed':'pending',errors:aiInspection.status==='blocked'?1:0,warnings:0};
  const gates=[mergeGate('생성 준비',[0,1]),printDocumentGate,corePdfGate,aiImageGate,mergeGate('외부·실물 승인',[4,5])];
