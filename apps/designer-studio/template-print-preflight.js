(()=>{
 ...
 function artifactReport(artifact){
  if(!artifact||typeof artifact!=='object')return null;
  const keys=['id','jobId','status','schemaVersion','verified','pageCount','byteLength','pipelineVersion','completedAt','resumedFrom','optimized','jpegQuality','imageResolutionDpi','checks','metrics','issues'];
  return Object.fromEntries(keys.filter(key=>artifact[key]!==undefined).map(key=>[key,artifact[key]]));
 }
 function analyze(project,{templateId=null,version=null,runtimeDocument=undefined,renderParity=undefined,printOutput=undefined}={}){
  ...
  const approvalStage=(name,value)=>({name,status:value?.status==='passed'?'passed':value?.status==='failed'?'blocked':'pending',errors:value?.status==='failed'?1:0,warnings:0});
  const mergeGate=(name,indexes)=>{const items=indexes.map(index=>stages[index]),states=items.map(item=>item.status);return {name,status:states.includes('blocked')?'blocked':states.includes('review')?'review':states.includes('pending')?'pending':'passed',errors:items.reduce((sum,item)=>sum+item.errors,0),warnings:items.reduce((sum,item)=>sum+item.warnings,0)}};
  const outputProblems=problems.slice(renderIssueCount),contractOutputProblems=outputProblems.filter(item=>!text(item.path).startsWith('print.artifact')),artifactOutputProblems=outputProblems.filter(item=>text(item.path).startsWith('print.artifact'));
  const printDocumentGate=printOutput===undefined?{name:'Print Document·화면',status:'pending',errors:0,warnings:0}:printOutput.contractReady===false?stage('Print Document·화면',contractOutputProblems):renderParity===undefined?{name:'Print Document·화면',status:'pending',errors:0,warnings:0}:stage('Print Document·화면',[...problems.slice(runtimeIssueCount,renderIssueCount),...contractOutputProblems]);
  const trimParityStatus=text(printOutput?.contentParity?.status),coreOutputReady=Boolean(printOutput?.artifactVerified)&&trimParityStatus==='passed';
  const corePdfGate=!printOutput?.artifact?{name:'핵심 자동검사',status:'pending',errors:0,warnings:0}:coreOutputReady?{name:'핵심 자동검사',status:'passed',errors:0,warnings:0}:stage('핵심 자동검사',artifactOutputProblems);
  const aiInspection=printOutput?.aiImageInspection||{status:'pending',criteria:[],result:null,legacyReview:null};
  const aiImageGate={name:'AI 생성 이미지 검사',status:aiInspection.status==='blocked'?'blocked':aiInspection.status==='passed'?'passed':'pending',errors:aiInspection.status==='blocked'?1:0,warnings:0,disposition:aiInspection.disposition||'required'};
  const internalInspectionComplete=!errors&&!warnings&&runtimeDocument!==undefined&&renderParity!==undefined&&coreOutputReady&&aiImageGate.status==='passed';
  const sourceApproval=printOutput?.approval||{};
  const approval=Object.freeze({automated:sourceApproval.automated||{status:printOutput?.artifactVerified?'passed':'pending',label:'자동 인쇄 Preflight'},external:{status:internalInspectionComplete?'passed':'not_run',label:'Acrobat 외부 Preflight (후속 권장)',required:false,autoCompleted:internalInspectionComplete},physical:{status:internalInspectionComplete?'passed':'not_run',label:'대표 실물 출력 (후속 권장)',required:false,autoCompleted:internalInspectionComplete},finalApproved:internalInspectionComplete,artifactClass:sourceApproval.artifactClass||(printOutput?.artifact?'legacy-converted':'not-generated')});
  const finalReviewPending=printOutput!==undefined&&!approval.finalApproved&&!errors;
  const stages=[stage('Package 계약',problems.slice(0,contractIssueCount)),stage('Runtime 문서',runtimeDocument===undefined?[issue('warning','NOT_RUN','실행 전')]:problems.slice(contractIssueCount,runtimeIssueCount)),renderParity===undefined?{name:'화면·RGB PDF',status:'pending',errors:0,warnings:0}:stage('화면·RGB PDF',problems.slice(runtimeIssueCount,renderIssueCount)),printOutput===undefined?{name:'자동 인쇄 Preflight',status:'pending',errors:0,warnings:0}:stage('자동 인쇄 Preflight',problems.slice(renderIssueCount)),approvalStage('외부 Preflight 후속 권장',approval.external),approvalStage('대표 실물 출력 후속 권장',approval.physical)];
  const finalApprovalGate=mergeGate('검사 완료',[4,5]),baseGates=[mergeGate('생성 준비',[0,1]),printDocumentGate,corePdfGate,aiImageGate,finalApprovalGate];
  const gates=[];baseGates.forEach((gate,index)=>{const prerequisite=index===3?2:index===4?3:null,prior=prerequisite===null?null:gates[prerequisite],available=prerequisite===null||(prior?.access==='available'&&prior?.status==='passed');gates.push({...gate,access:available?'available':'locked',blockedBy:available?null:prerequisite+1})});
  return Object.freeze({schemaVersion:'template-preflight-report.v6',generatedAt:new Date().toISOString(),identity:{templateId:text(template.id||templateId)||null,version:text(template.version||version)||null},status:errors?'blocked':warnings||finalReviewPending||(printOutput!==undefined&&aiImageGate.status!=='passed')?'review':'passed',approvalLevel:approval.finalApproved?'final-approved':coreOutputReady?'core-print-preflight':'composition-check',finalApproved:approval.finalApproved===true,approval,summary:{pages:pages.length,pageRoles:pageRoles.size,elements:[...elementTypes.values()].reduce((sum,count)=>sum+count,0),elementTypes:elementTypes.size,styles:styleKeys.size,bindings:bindings.size,assets:assets.size,errors,warnings,remainingImprovements:printOutput===undefined?0:aiImageGate.status==='passed'?0:1,followUpChecks:listOf(printOutput?.runtimeArtifactChecks).length,issueGroups:groupIssues(problems).length},stages,gates,coreOutput:{status:corePdfGate.status,artifactVerified:Boolean(printOutput?.artifactVerified),trimContentParity:trimParityStatus||'not_run'},aiImageInspection:aiInspection,userServiceInspection:{status:'deferred',checks:listOf(printOutput?.runtimeArtifactChecks)},runtime:runtimeDocument===undefined?null:{generated:Boolean(runtimeDocument),pages:listOf(runtimeDocument?.pages).length,objects:listOf(runtimeDocument?.pages).reduce((sum,page)=>sum+listOf(page.objects).length,0),diagnostics:listOf(runtimeDocument?.diagnostics).length,version:text(runtimeDocument?.runtimeVersion)||null},renderParity:renderParity===undefined?null:{generated:Boolean(renderParity?.generated),pages:Number(renderParity?.pages)||0,screenObjects:Number(renderParity?.screenObjects)||0,rgbObjects:Number(renderParity?.rgbObjects)||0},printOutput:printOutput===undefined?null:{contractReady:Boolean(printOutput?.contractReady),artifactVerified:Boolean(printOutput?.artifactVerified),artifactChecksComplete:Boolean(printOutput?.artifactChecksComplete),failedArtifactChecks:listOf(printOutput?.failedArtifactChecks),missingArtifactChecks:listOf(printOutput?.missingArtifactChecks),contentParity:printOutput?.contentParity||null,artifact:artifactReport(printOutput?.artifact),worker:text(printOutput?.worker)||null,profile:printOutput?.profile||null},inventory:{pageRoles:Object.fromEntries(pageRoles),elementTypes:Object.fromEntries(elementTypes),styleKeys:Object.fromEntries(styleKeys),bindings:[...bindings].sort(),capabilities:[...capabilities].sort()},issueGroups:groupIssues(problems),issues:problems});
 }
