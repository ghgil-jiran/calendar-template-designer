(()=>{
 ...
 async function inspectDeploy(event){
  ...
  const remote=window.ACDLTemplateRemotePersistence;if(!remote?.isRemote?.())throw new Error('사용자 서비스 배포는 원격 저장이 연결된 환경에서만 가능합니다.');const registering=button.dataset.stage==='register-review'&&pendingReviewIdentity?.packageId===packageId&&pendingReviewIdentity?.packageVersion===packageVersion,previousLabel=button.textContent;button.disabled=true;button.textContent=registering?'검토본 등록 중…':'배포 정보 확인 중…';feedback.className='save-feedback info';feedback.textContent=registering?'큰 Package를 안전하게 나누어 전송하고 SHA를 검증하고 있습니다. 창을 닫지 마세요.':'저장 버전과 Package 구성을 검사하고 있습니다.';
