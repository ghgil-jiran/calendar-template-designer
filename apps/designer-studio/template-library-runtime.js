(()=>{
 ...
 async function openPrintPreflight(record){
  if(!record)return;preflightRecord=record;preflightReport=null;preflightFilter='all';const dialog=el('templatePreflightDialog'),status=el('templatePreflightStatus'),download=el('downloadTemplatePreflightBtn');dialog.classList.remove('hidden');download&&(download.disabled=true);dialog.querySelectorAll('[data-preflight-filter]').forEach(button=>button.classList.toggle('active',button.dataset.preflightFilter==='all'));status.className='template-preflight-status working';status.innerHTML='<strong>검사 중</strong><span>저장된 템플릿의 전체 페이지와 개체를 확인하고 있습니다.</span>';el('templatePreflightTitle').textContent=`${record.name} · 인쇄 품질 검사`;el('templatePreflightDescription').textContent=`${record.type} · ${record.edition} Edition · ${internalVersionLabel(record)}`;el('templatePreflightMetrics').innerHTML='';el('templatePreflightIssues').innerHTML='<p class="template-preflight-empty">검사 결과를 준비하고 있습니다.</p>';el('templatePreflightInventory').innerHTML='';
  try{const project=await projectForRecord(record),report=window.ACDLTemplatePrintPreflight.analyze(project,{templateId:record.stableKey||record.id,version:record.packageVersion||record.version});preflightReport=report;download&&(download.disabled=false);const labels={passed:['검사 통과','현재 기본 검사에서 막히는 항목이 없습니다.'],review:['확인 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.warnings}개 경고를 확인해 주세요.`],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;el('templatePreflightMetrics').innerHTML=[['페이지',report.summary.pages],['개체',report.summary.elements],['개체 종류',report.summary.elementTypes],['스타일',report.summary.styles],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${value}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();el('templatePreflightInventory').innerHTML=`<h4>페이지 역할</h4><div>${preflightCount(report.inventory.pageRoles)}</div><h4>개체 종류</h4><div>${preflightCount(report.inventory.elementTypes)}</div><h4>스타일 속성</h4><div>${preflightCount(report.inventory.styleKeys)}</div>`;record.preflightReport=report
