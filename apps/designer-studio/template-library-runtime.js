(()=>{
 ...
 async function hydrateThumbnails(list){const markers=[],pending=[];list.forEach(record=>{const host=document.querySelector(`[data-library-thumbnail="${CSS.escape(record.id)}"]`);if(!host)return;const uploaded=record.thumbnail?.kind==='upload'?record.thumbnail:null,src=uploaded?.dataUrl||'',match=String(src).match(/^acdl-asset:\/\/([0-9a-f-]{36})$/i);if(match){markers.push(match[1]);pending.push({record,host,assetId:match[1]});return}if(src){host.innerHTML=`<img class="library-uploaded-thumbnail" src="${src}" alt="${escape(record.name)} 대표 이미지">`;host.dataset.rendered='true';return}if(record.source==='catalog'){renderActualThumbnail(record,host);return}host.innerHTML='<span class="thumbnail-placeholder">대표 이미지 저장 필요</span>';host.dataset.rendered='missing'});if(!markers.length)return;try{const urls=await window.ACDLTemplateRemotePersistence?.resolveAssetUrls?.(markers)||{};pending.forEach(({record,host,assetId})=>{if(!host.isConnected)return;const src=urls[assetId];if(src){host.innerHTML=`<img class="library-uploaded-thumbnail" src="${src}" alt="${escape(record.name)} 대표 이미지">`;host.dataset.rendered='true'}else{host.innerHTML='<span class="thumbnail-placeholder">대표 이미지를 불러오지 못했습니다.</span>';host.dataset.rendered='missing'}})}catch(error){pending.forEach(({host})=>{if(host.isConnected){host.innerHTML='<span class="thumbnail-placeholder">대표 이미지를 불러오지 못했습니다.</span>';host.dataset.rendered='missing'}});console.warn('Template thumbnail batch failed',error)}}
