(()=>{
 ...
 function openDeploy(record){
  ...
  pendingDeploy=record;pendingReviewIdentity=null;const packageId=record.classification?.packageId||record.packageId||'',packageVersion=record.packageVersion||'1.0.0';el('deployTemplateId').value=packageId;el('deployTemplateVersion').value=packageVersion;const feedback=el('templateDeployFeedback'),button=el('confirmTemplateDeployBtn');feedback.className=packageId?'save-feedback hidden':'save-feedback error';feedback.textContent=packageId?'':'시스템 베이스의 Package ID가 없습니다. 최종 분류를 다시 확인해 주세요.';button.disabled=!packageId;delete button.dataset.stage;button.textContent='검토용 Package 생성';el('templateDeployDialog').classList.remove('hidden');button.focus();
 }
 ...
 function renderLibrary(filter='all'){
  ...
  if(!list.length && activeLibraryScope==='custom'){
    ...
    grid.querySelectorAll('[data-library-deploy]').forEach(button=>button.addEventListener('click',()=>{try{openDeploy(records().find(record=>record.id===button.dataset.libraryDeploy))}catch(error){showEditorToast(error?.message||'배포 화면을 열지 못했습니다.')}}));
