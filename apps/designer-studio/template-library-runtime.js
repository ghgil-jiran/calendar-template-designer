...
async function runFinalPreflight(){
  ...
  try{const requestedArtifact=normalizedArtifact(await window.ACDLTemplatePublishing.ensurePrintPreflight(preflightIdentity,{force:true}));await loadPreflightHistory();preflightArtifact=latestPreflightArtifact(preflightHistory)||requestedArtifact;renderCurrentPreflight(preflightArtifact?.status==='done'?'완료된 최종 PDF 검사 결과를 불러왔습니다.':'최종 PDF 작업이 대기열에 등록됐습니다.')}catch(error){status.className='template-preflight-status blocked';status.innerHTML=`<strong>요청 실패</strong><span>${escape(error?.message||'최종 PDF 검사를 요청하지 못했습니다.')}</span>`}finally{setPreflightBusy('runTemplateFinalPreflightBtn',false);setPreflightButtonLabel('runTemplateFinalPreflightBtn',preflightArtifact?.status==='done'?'검사 완료 · 다시 검사':['queued','processing'].includes(preflightArtifact?.status)?'중단하고 다시 검사':'최종 PDF 생성·검사');el('runTemplateFinalPreflightBtn').disabled=false}
 }
