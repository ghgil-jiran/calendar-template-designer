(()=>{
 ...
 function artifactCheck(artifact,key){const checks=artifact?.checks||artifact?.preflight?.checks||artifact?.details||{},aliases={pdfx4:['pdfx4','pdfX4','pdfStandard'],outputIntent:['outputIntent','icc','colorProfile'],cmyk:['cmyk','colorMode'],k100:['k100','black'],trimBox:['trimBox','trimbox'],bleedBox:['bleedBox','bleedbox'],fontOutline:['fontOutline','fontsOutlined','outline']}[key]||[key];let value=null;for(const name of aliases){value=checks[name]??artifact?.[name];if(value!==undefined&&value!==null)break}if(value&&typeof value==='object')return value.passed??value.valid??value.status==='passed';if(value===undefined||value===null)return artifact?.verified===true;return value===true||value==='passed'||value==='valid'||value==='PDF/X-4'||value==='CMYK'||value==='K100'||value==='outline'}
 function artifactFilename(identity,artifact){const stamp=String(artifactValue(artifact,'completedAt','finishedAt','updatedAt')||new Date().toISOString()).replace(/[-:]/g,'').replace(/\.\d{3}Z$/,'Z').replace('T','-'),id=identity?.templateId||'template',version=identity?.version||'unknown';return `${id}-${version}-${stamp}.pdf`}
 ...
 function renderPreflightArtifact(identity,artifact){const host=el('templatePreflightArtifact'),button=el('downloadTemplatePrintPdfBtn');preflightArtifact=artifact||null;if(button)button.disabled=!artifact||artifact.status!=='done'||artifact.verified!==true;if(!host)return;if(!artifact){host.innerHTML='<h3>최종 인쇄 PDF</h3><p>완료된 CMYK PDF 산출물이 없습니다.</p>';return}const checks=[['PDF/X-4','pdfx4'],['ICC·OutputIntent','outputIntent'],['CMYK','cmyk'],['K100','k100'],['TrimBox','trimBox'],['BleedBox','bleedBox'],['서체 아웃라인','fontOutline']];host.innerHTML=`<h3>최종 인쇄 PDF</h3><div class="template-preflight-artifact-summary"><strong>${artifact.verified===true?'자동검사 통과':artifact.status==='processing'?'생성 중':artifact.status==='queued'?'처리 대기':'검사 실패'}</strong><div><span>${escape(artifactFilename(identity,artifact))}</span><small>잡 ${escape(artifact.id||artifact.jobId||'-')} · ${formatArtifactTime(artifactValue(artifact,'completedAt','finishedAt','updatedAt'))}</small></div></div><div class="template-preflight-artifact-grid">${checks.map(([label,key])=>{const passed=artifactCheck(artifact,key);return `<div class="${passed?'passed':'blocked'}"><span>${label}</span><strong>${passed?'통과':'확인 필요'}</strong></div>`}).join('')}<div class="${Number(artifactValue(artifact,'pageCount','pages'))>0?'passed':'blocked'}"><span>페이지 수</span><strong>${escape(artifactValue(artifact,'pageCount','pages')||'-')}면</strong></div><div class="passed"><span>파일 크기</span><strong>${formatArtifactBytes(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))}</strong></div></div>`}
 ...
 async function openPrintPreflight(record){
  ...
  try{
   ...
   const identity=inspectedProject?.template?.publishing?.lastReviewPackage;let artifact=null,history=[];
