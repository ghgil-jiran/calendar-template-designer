(()=>{
 ...
 async function openPrintPreflight(record){
  ...
  try{const project=await projectForRecord(record),runtimeDocument=window.ACDLRuntimeBridge?.resolve?.(project)||null,report=window.ACDLTemplatePrintPreflight.analyze(project,{templateId:record.stableKey||record.id,version:record.packageVersion||record.version,runtimeDocument});preflightReport=report;download&&(download.disabled=false);const labels={passed:['검사 통과','Package 계약과 공통 Runtime 문서 변환을 통과했습니다.'],review:['확인 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.warnings}개 경고를 확인해 주세요.`],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;renderPreflightStages(report);el('templatePreflightMetrics').innerHTML=[['원본 페이지',report.summary.pages],['원본 개체',report.summary.elements],['Runtime 페이지',report.runtime?.pages||0],['Runtime 개체',report.runtime?.objects||0],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${value}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();el('templatePreflightInventory').innerHTML=`<h4>Runtime 결과</h4><div><span><b>${escape(report.runtime?.version||'미생성')}</b></span><span>진단 <b>${report.runtime?.diagnostics||0}</b></span></div><h4>페이지 역할</h4><div>${preflightCount(report.inventory.pageRoles)}</div><h4>개체 종류</h4><div>${preflightCount(report.inventory.elementTypes)}</div><h4>스타일 속성</h4><div>${preflightCount(report.inventory.styleKeys)}</div>`;record.preflightReport=report
