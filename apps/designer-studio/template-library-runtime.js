(()=>{
 ...
 function renderPreflightStages(report){const labels={passed:'통과',review:'확인',blocked:'중단',pending:'예정'},source=report.stages||[],merge=(name,items)=>{const states=items.map(item=>item?.status),errors=items.reduce((sum,item)=>sum+(item?.errors||0),0),warnings=items.reduce((sum,item)=>sum+(item?.warnings||0),0);return {name,status:states.includes('blocked')?'blocked':states.includes('review')?'review':states.includes('pending')?'pending':'passed',errors,warnings}},gates=report.gates||[merge('생성 준비',[source[0],source[1]]),merge('Print Document·화면',[source[2]]),merge('최종 PDF 자동검사',[source[3]]),merge('외부·실물 승인',[source[4],source[5]])];el('templatePreflightStages').innerHTML=gates.map((stage,index)=>`<div class="${stage.status}"><span>${index+1}단계</span><strong>${escape(stage.name)}</strong><small>${labels[stage.status]}${stage.errors||stage.warnings?` · 오류 ${stage.errors} / 경고 ${stage.warnings}`:''}</small></div>`).join('')}
 ...
 function renderPreflightArtifact(identity,artifact){const host=el('templatePreflightArtifact'),button=el('downloadTemplatePrintPdfBtn');preflightArtifact=artifact||null;if(button)button.disabled=!artifact||artifact.status!=='done';if(!host)return;if(!artifact){host.innerHTML='<h3>자동검사 산출물 PDF</h3><p>완료된 CMYK PDF 산출물이 없습니다.</p>';return}const checks=[['PDF/X-4','pdfx4'],['ICC·OutputIntent','outputIntent'],['CMYK','cmyk'],['K100','k100'],['TrimBox','trimBox'],['BleedBox','bleedBox'],['서체 아웃라인','fontOutline'],['벡터·래스터 구조','vectorContentPreserved'],['재단영역 내용 동등성','trimContentParity'],['이미지 실배치 300DPI','imageDpi'],['템플릿 기본 이미지 승인','templateImageApproval'],['최종 인쇄 이미지 승인','finalPrintImageApproval']],results=checks.map(([label,key])=>[label,key,artifactCheck(artifact,key)]),failed=results.filter(([, ,result])=>result.status==='failed'),notRun=results.filter(([, ,result])=>!['passed','failed'].includes(result.status)),summary=failed.length?'품질검사 실패':notRun.length?'검사 진행 중 · 미완료 항목 있음':artifact.verified===true?'필수 자동검사 통과':'검사 결과 확인 필요';host.innerHTML=`<h3>자동검사 산출물 PDF</h3><div class="template-preflight-artifact-summary"><strong>${summary}</strong><div><span>${escape(artifactFilename(identity,artifact))}</span><small>잡 ${escape(artifact.id||artifact.jobId||'-')} · ${formatArtifactTime(artifactValue(artifact,'completedAt','finishedAt','updatedAt'))}</small></div></div><div class="template-preflight-artifact-grid">${results.map(([label,key,result])=>{const state=result.status==='passed'?'passed':result.status==='failed'?'blocked':'pending',title=result.status==='passed'?'통과':result.status==='failed'?'실패':result.status==='external_required'?'외부검증 필요':key==='trimContentParity'?'동일 Dataset 비교 필요':'검사 전';return `<div class="${state}"><span>${label}</span><strong>${title}</strong>${result.message?`<small>${escape(result.message)}</small>`:''}</div>`}).join('')}<div class="${Number(artifactValue(artifact,'pageCount','pages'))>0?'passed':'blocked'}"><span>페이지 수</span><strong>${escape(artifactValue(artifact,'pageCount','pages')||'-')}면</strong></div><div class="passed"><span>파일 크기</span><strong>${formatArtifactBytes(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))}</strong></div><div class="pending"><span>Acrobat 외부 Preflight</span><strong>검사 전</strong><small>자동검사와 별도로 확인해야 합니다.</small></div><div class="pending"><span>실물 인쇄 승인</span><strong>승인 전</strong><small>대표 템플릿 출력 후 최종 승인합니다.</small></div></div>`}
 ...
 function setPreflightBusy(buttonId,busy,text){const button=el(buttonId);if(!button)return;button.disabled=busy;if(text){if(!button.dataset.idleText)button.dataset.idleText=button.textContent;button.textContent=busy?text:button.dataset.idleText}}
 ...
 function renderCurrentPreflight(message){
  ...
  const labels={passed:['최종 인쇄 승인','자동검사와 외부·실물 승인이 모두 완료됐습니다.'],review:['검사 단계 선택',message||'빠른 검사는 완료됐습니다. 필요할 때 화면 확인 또는 최종 PDF 검사를 실행하세요.'],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;
  ...
  el('runTemplateFinalPreflightBtn').disabled=report.summary.errors>0;preflightRecord.preflightReport=report;
 }
 ...
 async function runQuickPreflight(){
  if(!preflightRecord)return;const status=el('templatePreflightStatus');setPreflightBusy('runTemplateQuickPreflightBtn',true,'검사 중');status.className='template-preflight-status working';status.innerHTML='<strong>빠른 검사 중</strong><span>Package·페이지·Runtime 계약을 확인합니다. PDF 작업은 만들지 않습니다.</span>';
  try{preflightProject=preflightProject||await projectForRecord(preflightRecord);preflightRuntimeDocument=window.ACDLRuntimeBridge?.resolve?.(preflightProject)||null;const packageIdentity=preflightProject?.template?.publishing?.lastReviewPackage;preflightIdentity=packageIdentity?{...packageIdentity,name:preflightRecord.name||preflightProject?.template?.metadata?.name}:null;await loadPreflightHistory();renderCurrentPreflight('생성 준비 검사를 완료했습니다. 기존 산출물만 조회했으며 새 PDF 작업은 만들지 않았습니다.')}catch(error){status.className='template-preflight-status blocked';status.innerHTML=`<strong>검사 중단</strong><span>${escape(error?.message||'템플릿을 불러오지 못했습니다.')}</span>`}finally{setPreflightBusy('runTemplateQuickPreflightBtn',false)}
 }
 async function runRenderPreflight(){
  if(!preflightProject)await runQuickPreflight();if(!preflightProject)return;const status=el('templatePreflightStatus');setPreflightBusy('runTemplateRenderCheckBtn',true,'확인 중');status.className='template-preflight-status working';status.innerHTML='<strong>화면·RGB 확인 중</strong><span>전체 페이지를 화면과 RGB PDF 경로로 비교합니다. 최종 CMYK PDF는 만들지 않습니다.</span>';
  try{preflightRenderParity=await captureRenderParity(preflightProject);renderCurrentPreflight(preflightRenderParity?.generated?'화면·RGB 비교를 완료했습니다.':'화면·RGB 비교 결과를 확인해 주세요.')}finally{setPreflightBusy('runTemplateRenderCheckBtn',false)}
 }
 async function runFinalPreflight(){
  if(!preflightProject)await runQuickPreflight();if(!preflightProject)return;if(preflightRenderParity===undefined)await runRenderPreflight();const preliminary=composePreflightReport(),status=el('templatePreflightStatus');if(preliminary.summary.errors){renderCurrentPreflight('선행 검사 오류를 먼저 수정해야 최종 PDF 작업을 생성할 수 있습니다.');return}if(!preflightIdentity?.templateId||!preflightIdentity?.version||!preflightIdentity?.sha256){renderCurrentPreflight('검토 Package 식별 정보가 없어 최종 PDF 작업을 생성할 수 없습니다.');return}
  ...
  try{const requestedArtifact=normalizedArtifact(await window.ACDLTemplatePublishing.ensurePrintPreflight(preflightIdentity));await loadPreflightHistory();preflightArtifact=latestPreflightArtifact(preflightHistory)||requestedArtifact;renderCurrentPreflight(preflightArtifact?.status==='done'?'완료된 최종 PDF 검사 결과를 불러왔습니다.':'최종 PDF 작업이 대기열에 등록됐습니다. 완료 결과 확인으로 상태를 갱신하세요.')}catch(error){status.className='template-preflight-status blocked';status.innerHTML=`<strong>요청 실패</strong><span>${escape(error?.message||'최종 PDF 검사를 요청하지 못했습니다.')}</span>`}finally{setPreflightBusy('runTemplateFinalPreflightBtn',false);if(preflightReport?.summary.errors)el('runTemplateFinalPreflightBtn').disabled=true}
 }
 async function refreshPreflightResult(){
  if(!preflightProject)return;setPreflightBusy('rerunTemplatePreflightBtn',true,'확인 중');try{await loadPreflightHistory();renderCurrentPreflight(preflightArtifact?.status==='done'?'완료 결과를 불러왔습니다.':'현재 작업 상태를 갱신했습니다. 새 작업은 만들지 않았습니다.')}finally{setPreflightBusy('rerunTemplatePreflightBtn',false)}
 }
 async function openPrintPreflight(record){
  if(!record)return;preflightRecord=record;preflightProject=null;preflightRuntimeDocument=null;preflightRenderParity=undefined;preflightIdentity=null;preflightHistory=[];preflightReport=null;preflightArtifact=null;preflightFilter='all';const dialog=el('templatePreflightDialog'),download=el('downloadTemplatePreflightBtn'),pdfDownload=el('downloadTemplatePrintPdfBtn');dialog.classList.remove('hidden');download&&(download.disabled=true);pdfDownload&&(pdfDownload.disabled=true);dialog.querySelectorAll('[data-preflight-filter]').forEach(button=>button.classList.toggle('active',button.dataset.preflightFilter==='all'));el('templatePreflightTitle').textContent=`${record.name} · 인쇄 품질 검사`;el('templatePreflightDescription').textContent=`${record.type} · ${record.edition} Edition · ${internalVersionLabel(record)}`;el('templatePreflightStages').innerHTML='';el('templatePreflightMetrics').innerHTML='';el('templatePreflightIssues').innerHTML='<p class="template-preflight-empty">빠른 검사를 준비하고 있습니다.</p>';el('templatePreflightInventory').innerHTML='';el('templatePreflightArtifact').innerHTML='<h3>자동검사 산출물 PDF</h3><p>기존 산출물을 확인합니다.</p>';el('templatePreflightHistory').innerHTML='<h3>최근 산출물과 검사 이력</h3><p>이력을 확인합니다.</p>';await runQuickPreflight()
 }
