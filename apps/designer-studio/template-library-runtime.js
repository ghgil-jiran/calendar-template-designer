(()=>{
 ...
 function renderAiImageInspection(report){
  ...
  const state=inspection.status==='passed'?'passed':inspection.status==='blocked'?'blocked':'pending',title=inspection.status==='passed'?'검사 완료':inspection.status==='blocked'?'개선 필요':'검사 준비 전';
  host.innerHTML=`<h3>AI 생성 이미지 검사</h3><div class="template-preflight-artifact-summary"><strong>${title}</strong><div><span>배경·일러스트 생성 기준과 결과를 검사합니다.</span><small>AI 이미지 생성 기능에 검사 기준이 먼저 반영되어야 합니다.</small></div></div><p class="template-preflight-artifact-wait">현재 단계는 AI 생성 기능을 개선한 뒤 실행합니다. 기준 적용 전 생성 이미지는 합격 판정에 사용하지 않습니다.</p><h4>검사 기준</h4><div class="template-preflight-artifact-grid">${criteria.map(item=>`<div class="${state}"><span>필수 검사</span><strong>${inspection.status==='passed'?'통과':'검사 대기'}</strong><small>${escape(item)}</small></div>`).join('')}</div>${legacy?`<h4>기존 참고 기록</h4><div class="template-preflight-artifact-grid"><div class="pending"><span>기준 적용 전 기록</span><strong>합격 판정 제외</strong><small>${escape(legacy.message||'이전 이미지 배치 검토 기록')}</small></div></div>`:''}<h4>후속 범위</h4><div class="template-preflight-artifact-grid"><div class="pending"><span>최종 템플릿 저장</span><strong>추후 연결</strong><small>업스케일 처리 기준은 최종 템플릿 저장 단계에서 별도로 연결합니다.</small></div><div class="pending"><span>사용자 이미지</span><strong>사용자 서비스에서 검사</strong><small>실효 DPI·재업로드·저해상도 승인 여부는 사용자 서비스 범위입니다.</small></div></div>`;
 }
