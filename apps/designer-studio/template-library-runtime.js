(()=>{
 ...
 async function openPrintPreflight(record){
  ...
  try{const inspectedProject=await projectForRecord(record),runtimeDocument=window.ACDLRuntimeBridge?.resolve?.(inspectedProject)||null;status.innerHTML='<strong>3단계 검사 중</strong><span>전체 페이지를 화면과 RGB PDF 출력 경로로 렌더링하고 있습니다.</span>';const renderParity=await captureRenderParity(inspectedProject),report=window.ACDLTemplatePrintPreflight.analyze(inspectedProject,{templateId:record.stableKey||record.id,version:record.packageVersion||record.version,runtimeDocument,renderParity});preflightReport=report;download&&(download.disabled=false);const labels={passed:['검사 통과','Package 계약, Runtime 문서, 화면·RGB PDF 동등성 검사를 통과했습니다.'],review:['확인 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.warnings}개 경고를 확인해 주세요.`],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;renderPreflightStages(report);el('templatePreflightMetrics').innerHTML=[['원본 페이지',report.summary.pages],['Runtime 페이지',report.runtime?.pages||0],['화면 페이지',report.renderParity?.pages||0],['RGB PDF 페이지',report.renderParity?.pages||0],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${value}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();el('templatePreflightInventory').innerHTML=`<h4>화면·RGB PDF 결과</h4><div><span>화면 개체 <b>${report.renderParity?.screenObjects||0}</b></span><span>RGB PDF 개체 <b>${report.renderParity?.rgbObjects||0}</b></span></div><h4>Runtime 결과</h4><div><span><b>${escape(report.runtime?.version||'미생성')}</b></span><span>진단 <b>${report.runtime?.diagnostics||0}</b></span></div><h4>페이지 역할</h4><div>${preflightCount(report.inventory.pageRoles)}</div><h4>개체 종류</h4><div>${preflightCount(report.inventory.elementTypes)}</div><h4>스타일 속성</h4><div>${preflightCount(report.inventory.styleKeys)}</div>`;record.preflightReport=report
