(()=>{
 ...
 function workerCommandMarkup(artifact){const active=['queued','processing'].includes(artifact?.status),resumable=artifact?.status==='error'&&artifact?.superseded!==true;if(!active&&!resumable)return '';const title=resumable?'실패 단계부터 재개':'이 잡만 실행',note=resumable?'로컬 산출물에서 가장 최근의 안전한 체크포인트를 찾아 이어서 처리합니다. 체크포인트가 없으면 새 검사를 요청해야 합니다.':'사용자 서비스 서버를 먼저 실행한 뒤, 사용자 서비스 폴더의 다른 PowerShell 창에서 붙여넣으세요.';return `<h4>${title}</h4><div class="template-preflight-artifact-summary"><code>${escape(workerPowerShellCommand(artifact))}</code><button id="copyTemplateWorkerCommandBtn" type="button">PowerShell 명령어 복사</button></div><p class="template-preflight-artifact-wait">${note}</p>`}
