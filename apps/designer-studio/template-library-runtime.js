(()=>{
 ...
 let preflightReport=null,preflightFilter='all';
 ...
 async function openPrintPreflight(record){
  if(!record)return;preflightRecord=record;preflightReport=null;preflightFilter='all';const dialog=el('templatePreflightDialog'),status=el('templatePreflightStatus'),download=el('downloadTemplatePreflightBtn');dialog.classList.remove('hidden');download&&(download.disabled=true);dialog.querySelectorAll('[data-preflight-filter]').forEach(button=>button.classList.toggle('active',button.dataset.preflightFilter==='all'));status.className='template-preflight-status working';status.innerHTML='<strong>검사 중</strong><span>저장된 템플릿의 전체 페이지와 개체를 Runtime 문서로 변환하고 있습니다.</span>';el('templatePreflightTitle').textContent=`${record.name} · 인쇄 품질 검사`;el('templatePreflightDescription').textContent=`${record.type} · ${record.edition} Edition · ${internalVersionLabel(record)}`;el('templatePreflightStages').innerHTML='';el('templatePreflightMetrics').innerHTML='';el('templatePreflightIssues').innerHTML='<p class="template-preflight-empty">검사 결과를 준비하고 있습니다.</p>';el('templatePreflightInventory').innerHTML='';
  try{const inspectedProject=await projectForRecord(record),runtimeDocument=window.ACDLRuntimeBridge?.resolve?.(inspectedProject)||null;status.innerHTML='<strong>3단계 검사 중</strong><span>전체 페이지를 화면과 RGB PDF 출력 경로로 렌더링하고 있습니다.</span>';const renderParity=await captureRenderParity(inspectedProject);status.innerHTML='<strong>4단계 검사 중</strong><span>운영 PDF 워커의 CMYK·PDF/X-4 산출물 상태를 확인하고 있습니다.</span>';const identity=inspectedProject?.template?.publishing?.lastReviewPackage;let artifact=null;if(identity?.templateId&&identity?.version&&identity?.sha256&&!renderParity?.issues?.some(item=>item.severity==='error')){const job=await window.ACDLTemplatePublishing.ensurePrintPreflight(identity);artifact=job.status==='done'?{...(job.report||{}),status:job.status,filePath:job.filePath}:job}const printOutput=window.ACDLPrintOutputPreflight?.inspect?.(inspectedProject,{artifact})||null,report=window.ACDLTemplatePrintPreflight.analyze(inspectedProject,{templateId:record.stableKey||record.id,version:record.packageVersion||record.version,runtimeDocument,renderParity,printOutput});preflightReport=report;download&&(download.disabled=false);const labels={passed:['검사 통과','Package부터 실제 CMYK·PDF/X-4 산출물까지 모든 검사를 통과했습니다.'],review:['인쇄 출력 확인 필요',report.printOutput?.contractReady&&!report.printOutput?.artifactVerified?(artifact?.status==='processing'?'운영 PDF 워커가 실제 CMYK PDF를 생성하고 있습니다. 다시 검사를 눌러 결과를 확인하세요.':artifact?.status==='queued'?'운영 PDF 워커 처리 대기열에 등록했습니다. 워커 실행 후 다시 검사해 주세요.':'인쇄 계약은 준비됐습니다. 운영 PDF 워커에서 실제 CMYK PDF 생성과 파일 Preflight가 필요합니다.'):`${report.summary.issueGroups}개 원인에서 ${report.summary.warnings}개 경고를 확인해 주세요.`],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;renderPreflightStages(report);el('templatePreflightMetrics').innerHTML=[['원본 페이지',report.summary.pages],['Runtime 페이지',report.runtime?.pages||0],['화면 페이지',report.renderParity?.pages||0],['RGB PDF 페이지',report.renderParity?.pages||0],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${value}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();const pp=report.printOutput?.profile||{};el('templatePreflightInventory').innerHTML=`<h4>CMYK·PDF/X-4 준비 상태</h4><div><span>계약 <b>${report.printOutput?.contractReady?'준비 완료':'수정 필요'}</b></span><span>실제 PDF <b>${report.printOutput?.artifactVerified?'검증 완료':artifact?.status==='processing'?'생성 중':artifact?.status==='queued'?'대기열 등록':'워커 생성 필요'}</b></span><span><b>${escape(pp.pdfStandard||'-')}</b></span><span><b>${escape(pp.colorProfile||'-')}</b></span><span>제작 <b>${pp.productionSize?.width||0}×${pp.productionSize?.height||0}mm</b></span><span>재단 <b>${pp.trimSize?.width||0}×${pp.trimSize?.height||0}mm</b></span><span>도련 <b>${pp.bleed?.top||0}mm</b></span><span>검정 <b>${escape(pp.blackRule||'-')}</b></span><span>서체 <b>${escape(pp.fontHandling||'-')}</b></span></div><h4>화면·RGB PDF 결과</h4><div><span>화면 개체 <b>${report.renderParity?.screenObjects||0}</b></span><span>RGB PDF 개체 <b>${report.renderParity?.rgbObjects||0}</b></span></div><h4>Runtime 결과</h4><div><span><b>${escape(report.runtime?.version||'미생성')}</b></span><span>진단 <b>${report.runtime?.diagnostics||0}</b></span></div><h4>페이지 역할</h4><div>${preflightCount(report.inventory.pageRoles)}</div><h4>개체 종류</h4><div>${preflightCount(report.inventory.elementTypes)}</div><h4>스타일 속성</h4><div>${preflightCount(report.inventory.styleKeys)}</div>`;record.preflightReport=report
