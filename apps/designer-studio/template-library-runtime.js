(()=>{
 ...
 let preflightReport=null,preflightArtifact=null,preflightFilter='all',preflightActiveGate=null,preflightStatusMessage='';
 ...
 function preflightIssueGate(item){const code=String(item?.code||'');if(code.startsWith('RENDER_')||code.startsWith('RGB_')||(code.startsWith('PRINT_')&&!code.includes('ARTIFACT')))return 1;if(code.startsWith('PDF_')||code.includes('ARTIFACT'))return 2;return 0}
 function renderPreflightIssues(){const host=el('templatePreflightIssues');if(!host||!preflightReport)return;const gateGroups=(preflightReport.issueGroups||[]).filter(item=>preflightIssueGate(item)===preflightActiveGate),groups=gateGroups.filter(item=>preflightFilter==='all'||item.severity===preflightFilter);host.innerHTML=groups.length?groups.map(item=>`<article class="${item.severity}"><i>${item.severity==='error'?'!':'△'}</i><div><strong>${escape(item.message)}${item.count>1?`<span class="issue-count">${item.count}곳</span>`:''}</strong><small>${escape(item.code)}</small>${item.paths?.length?`<details><summary>영향 위치 ${item.count}곳 중 ${item.paths.length}곳 보기</summary>${item.paths.map(path=>`<code>${escape(path)}</code>`).join('')}</details>`:''}</div></article>`).join(''):`<p class="template-preflight-empty ${gateGroups.length?'':'success'}">${gateGroups.length?'선택한 항목이 없습니다.':preflightActiveGate===3?'외부 Preflight와 대표 실물 출력 결과를 등록하는 단계입니다.':'현재 단계에서 발견된 오류나 경고가 없습니다.'}</p>`}
 function renderPreflightStages(report){const labels={passed:'완료',review:'확인 필요',blocked:'수정 필요',pending:'미진행'},gates=report.gates||[];el('templatePreflightStages').innerHTML=gates.map((stage,index)=>`<button type="button" data-preflight-gate="${index}" class="${stage.status} ${index===preflightActiveGate?'active':''}" aria-current="${index===preflightActiveGate?'step':'false'}"><span>${index+1}단계</span><strong>${escape(stage.name)}</strong><small>${labels[stage.status]}${stage.errors||stage.warnings?` · 오류 ${stage.errors} / 경고 ${stage.warnings}`:''}</small></button>`).join('')}
 ...
 function renderPreflightArtifact(identity,artifact){const host=el('templatePreflightArtifact'),button=el('downloadTemplatePrintPdfBtn');preflightArtifact=artifact||null;if(button)button.disabled=!artifact||artifact.status!=='done';if(!host)return;if(!artifact){host.innerHTML='<h3>최종 PDF 자동검사</h3><p>아직 생성된 CMYK PDF가 없습니다. 선행 단계 통과 후 최종 PDF 생성·검사를 실행하세요.</p>';return}const checks=[['PDF/X-4','pdfx4'],['ICC·OutputIntent','outputIntent'],['CMYK','cmyk'],['K100','k100'],['TrimBox','trimBox'],['BleedBox','bleedBox'],['서체 아웃라인','fontOutline'],['벡터·래스터 구조','vectorContentPreserved'],['재단영역 내용 동등성','trimContentParity'],['이미지 실배치 300DPI','imageDpi'],['템플릿 기본 이미지 승인','templateImageApproval'],['최종 인쇄 이미지 승인','finalPrintImageApproval']],results=checks.map(([label,key])=>[label,key,artifactCheck(artifact,key)]),failed=results.filter(([, ,result])=>result.status==='failed'),notRun=results.filter(([, ,result])=>!['passed','failed'].includes(result.status)),summary=artifact.status==='queued'?'운영 PDF 워커 처리 대기':artifact.status==='processing'?'최종 PDF 생성·검사 중':failed.length?'품질검사 실패':notRun.length?'미완료 검사 항목 있음':artifact.verified===true?'필수 자동검사 통과':'검사 결과 확인 필요';host.innerHTML=`<h3>최종 PDF 자동검사</h3><div class="template-preflight-artifact-summary"><strong>${summary}</strong><div><span>${escape(artifactFilename(identity,artifact))}</span><small>잡 ${escape(artifact.id||artifact.jobId||'-')} · ${formatArtifactTime(artifactValue(artifact,'completedAt','finishedAt','updatedAt','createdAt'))}</small></div></div>${['queued','processing'].includes(artifact.status)?'<p class="template-preflight-artifact-wait">워커 처리가 완료되면 아래 자동검사 결과가 갱신됩니다.</p>':''}<div class="template-preflight-artifact-grid">${results.map(([label,key,result])=>{const state=result.status==='passed'?'passed':result.status==='failed'?'blocked':'pending',title=result.status==='passed'?'통과':result.status==='failed'?'실패':result.status==='external_required'?'외부검증 필요':key==='trimContentParity'?'동일 Dataset 비교 필요':'검사 전';return `<div class="${state}"><span>${label}</span><strong>${title}</strong>${result.message?`<small>${escape(result.message)}</small>`:''}</div>`}).join('')}<div class="${Number(artifactValue(artifact,'pageCount','pages'))>0?'passed':'pending'}"><span>페이지 수</span><strong>${escape(artifactValue(artifact,'pageCount','pages')||'-')}면</strong></div><div class="${Number(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))>0?'passed':'pending'}"><span>파일 크기</span><strong>${formatArtifactBytes(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))}</strong></div></div>`}
 function renderPreflightHistory(history,identity,artifact){const host=el('templatePreflightHistory');if(!host)return;const items=Array.isArray(history)?history:Array.isArray(history?.jobs)?history.jobs:Array.isArray(history?.history)?history.history:artifact?[artifact]:[];host.innerHTML=`<h3>최근 산출물과 검사 이력</h3>${items.length?`<div class="template-preflight-history-list">${items.slice(0,8).map(item=>`<article><strong>${escape(artifactFilename(identity,item))}</strong><span>${escape(item?.report?.superseded?'완료본 재사용으로 종료':item.status||'-')}</span><span>${formatArtifactBytes(artifactValue(item,'fileSize','fileSizeBytes','byteLength','size'))}</span><small>${formatArtifactTime(artifactValue(item,'completedAt','finishedAt','updatedAt','createdAt'))}</small></article>`).join('')}</div>`:'<p>저장된 검사 이력이 없습니다.</p>'}`}
 ...
 function normalizedArtifact(job){return job?.report?{...job.report,id:job.id||job.report.id,status:job.status,filePath:job.filePath,error:job.error,downloadUrl:job.downloadUrl||job.report.downloadUrl}:job||null}
 function latestPreflightArtifact(history){return historyItems(history).map(normalizedArtifact).sort((a,b)=>String(artifactValue(b,'completedAt','finishedAt','updatedAt','createdAt')||'').localeCompare(String(artifactValue(a,'completedAt','finishedAt','updatedAt','createdAt')||'')))[0]||null}
 ...
 function suggestedPreflightGate(report){const gates=report.gates||[];if(preflightArtifact&&['queued','processing'].includes(preflightArtifact.status))return 2;const blocked=gates.findIndex(item=>item.status==='blocked');if(blocked>=0)return blocked;const next=gates.findIndex(item=>item.status!=='passed');return next>=0?next:3}
 function renderCurrentPreflight(message){
  ...
  const gate=report.gates?.[preflightActiveGate]||{name:'검사 준비',status:'pending'},badgeLabels={passed:'완료',review:'확인 필요',blocked:'수정 필요',pending:'미진행'},defaultMessages=['Package·페이지·Runtime 계약을 검사합니다.','전체 페이지의 Print Document와 화면·RGB 결과를 비교합니다.','최종 CMYK PDF 생성 상태와 자동검사 결과를 확인합니다.','Acrobat 외부 Preflight와 대표 실물 인쇄 승인을 확인합니다.'];el('templatePreflightCurrentStep').textContent=`${preflightActiveGate+1}단계`;el('templatePreflightCurrentTitle').textContent=gate.name;const badge=el('templatePreflightCurrentBadge');badge.className=gate.status;badge.textContent=preflightActiveGate===2&&preflightArtifact?.status==='queued'?'처리 대기':preflightActiveGate===2&&preflightArtifact?.status==='processing'?'처리 중':badgeLabels[gate.status];status.className=`template-preflight-status ${gate.status==='blocked'?'blocked':gate.status==='passed'?'passed':gate.status==='review'?'review':'working'}`;status.innerHTML=`<strong>${escape(preflightStatusMessage||defaultMessages[preflightActiveGate])}</strong><span>${escape(defaultMessages[preflightActiveGate])}</span>`;
  renderPreflightStages(report);el('templatePreflightMetrics').innerHTML=[['원본 페이지',report.summary.pages],['Runtime 페이지',report.runtime?.pages||0],['화면 확인',report.renderParity?.pages||'미실행'],['최종 PDF',preflightArtifact?.status||'미생성'],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${escape(value)}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();renderPreflightArtifact(preflightIdentity,preflightArtifact);renderPreflightHistory(preflightHistory,preflightIdentity,preflightArtifact);
...
async function runFinalPreflight(){
  if(!preflightProject)await runQuickPreflight();if(!preflightProject)return;if(preflightRenderParity===undefined)await runRenderPreflight();preflightActiveGate=2;const preliminary=composePreflightReport(),status=el('templatePreflightStatus');if(preliminary.summary.errors){renderCurrentPreflight('선행 검사 오류를 먼저 수정해야 합니다.');return}if(!preflightIdentity?.templateId||!preflightIdentity?.version||!preflightIdentity?.sha256){renderCurrentPreflight('검토 Package 식별 정보가 없습니다.');return}
  setPreflightBusy('runTemplateFinalPreflightBtn',true,'요청 중…');status.className='template-preflight-status working';status.innerHTML='<strong>최종 PDF 검사 요청 중</strong><span>기존 완료 산출물을 우선 재사용하고, 필요한 경우에만 워커 작업을 생성합니다.</span>';
  try{const requestedArtifact=normalizedArtifact(await window.ACDLTemplatePublishing.ensurePrintPreflight(preflightIdentity));await loadPreflightHistory();preflightArtifact=latestPreflightArtifact(preflightHistory)||requestedArtifact;renderCurrentPreflight(preflightArtifact?.status==='done'?'완료된 최종 PDF 검사 결과를 불러왔습니다.':'최종 PDF 작업이 대기열에 등록됐습니다.')}catch(error){status.className='template-preflight-status blocked';status.innerHTML=`<strong>요청 실패</strong><span>${escape(error?.message||'최종 PDF 검사를 요청하지 못했습니다.')}</span>`}finally{setPreflightBusy('runTemplateFinalPreflightBtn',false);setPreflightButtonLabel('runTemplateFinalPreflightBtn',preflightArtifact?.status==='done'?'검사 완료 · 기존 PDF 재사용':preflightArtifact?.status==='processing'?'PDF 처리 중':preflightArtifact?.status==='queued'?'처리 대기 중':'최종 PDF 생성·검사');if(preflightReport?.summary.errors||['queued','processing'].includes(preflightArtifact?.status))el('runTemplateFinalPreflightBtn').disabled=true}
 }
async function refreshPreflightResult(){
  if(!preflightProject)return;preflightActiveGate=2;setPreflightBusy('rerunTemplatePreflightBtn',true,'확인 중…');try{await loadPreflightHistory();renderCurrentPreflight(preflightArtifact?.status==='done'?'완료 결과를 불러왔습니다.':'현재 작업 상태를 갱신했습니다. 새 작업은 만들지 않았습니다.')}finally{setPreflightBusy('rerunTemplatePreflightBtn',false)}
 }
 async function openPrintPreflight(record){
  if(!record)return;preflightRecord=record;preflightProject=null;preflightRuntimeDocument=null;preflightRenderParity=undefined;preflightIdentity=null;preflightHistory=[];preflightReport=null;preflightArtifact=null;preflightFilter='all';preflightActiveGate=0;preflightStatusMessage='';const dialog=el('templatePreflightDialog'),download=el('downloadTemplatePreflightBtn'),pdfDownload=el('downloadTemplatePrintPdfBtn');dialog.classList.remove('hidden');download&&(download.disabled=true);pdfDownload&&(pdfDownload.disabled=true);dialog.querySelectorAll('[data-preflight-filter]').forEach(button=>button.classList.toggle('active',button.dataset.preflightFilter==='all'));el('templatePreflightTitle').textContent=`${record.name} · 인쇄 품질 검사`;el('templatePreflightDescription').textContent=`${record.type} · ${record.edition} Edition · ${internalVersionLabel(record)}`;el('templatePreflightStages').innerHTML='<button class="active pending"><span>1단계</span><strong>생성 준비</strong><small>검사 준비</small></button><button class="pending"><span>2단계</span><strong>Print Document·화면</strong><small>미진행</small></button><button class="pending"><span>3단계</span><strong>최종 PDF 자동검사</strong><small>미진행</small></button><button class="pending"><span>4단계</span><strong>외부·실물 승인</strong><small>미진행</small></button>';el('templatePreflightMetrics').innerHTML='';el('templatePreflightIssues').innerHTML='<p class="template-preflight-empty">생성 준비 검사를 시작합니다.</p>';el('templatePreflightInventory').innerHTML='';el('templatePreflightArtifact').classList.add('hidden');el('templatePreflightHistory').innerHTML='<h3>최근 산출물과 검사 이력</h3><p>이력을 확인합니다.</p>';await runQuickPreflight()
 }
 ...
 ['closeTemplatePreflightBtn','closeTemplatePreflightFooterBtn'].forEach(id=>el(id)?.addEventListener('click',()=>el('templatePreflightDialog')?.classList.add('hidden')));el('runTemplateQuickPreflightBtn')?.addEventListener('click',runQuickPreflight);el('runTemplateRenderCheckBtn')?.addEventListener('click',runRenderPreflight);el('runTemplateFinalPreflightBtn')?.addEventListener('click',runFinalPreflight);el('rerunTemplatePreflightBtn')?.addEventListener('click',refreshPreflightResult);
 el('templatePreflightStages')?.addEventListener('click',event=>{const button=event.target.closest('[data-preflight-gate]');if(!button||!preflightReport)return;preflightActiveGate=Number(button.dataset.preflightGate);preflightStatusMessage='';renderCurrentPreflight()});
