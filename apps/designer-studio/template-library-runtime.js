(()=>{
 ...
 function cardMarkup(record){
  ...
  const remoteHistory=record.source==='local'&&record.storage==='supabase'?`<button data-library-history="${escape(record.id)}" aria-expanded="false">버전 이력</button><button data-library-package-check="${escape(record.id)}">Package 검사</button>`:'';
