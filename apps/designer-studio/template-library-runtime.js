(()=>{
 ...
 function renderPreflightArtifact(identity,artifact){const host=el('templatePreflightArtifact'),button=el('downloadTemplatePrintPdfBtn');preflightArtifact=artifact||null;if(button)button.disabled=!artifact||artifact.status!=='done'||artifact.verified!==true;if(!host)return;if(!artifact){host.innerHTML='<h3>최종 인쇄 PDF</h3><p>완료된 CMYK PDF 산출물이 없습니다.</p>';return}const checks=[['PDF/X-4','pdfx4'],['ICC·OutputIntent','outputIntent'],['CMYK','cmyk'],['K100','k100'],['TrimBox','trimBox'],['BleedBox','bleedBox'],['서체 아웃라인','fontOutline'],['벡터·래스터 구조','vectorContentPreserved'],['재단영역 내용 동등성','trimContentParity']],checksComplete=checks.every(([,key])=>artifactCheck(artifact,key)===true),summary=artifact.verified===true?(checksComplete?'필수 자동검사 통과':'기본검사 통과 · 추가 검사 필요'):artifact.status==='processing'?'생성 중':artifact.status==='queued'?'처리 대기':'검사 실패';host.innerHTML=`<h3>최종 인쇄 PDF</h3><div class="template-preflight-artifact-summary"><strong>${summary}</strong><div><span>${escape(artifactFilename(identity,artifact))}</span><small>잡 ${escape(artifact.id||artifact.jobId||'-')} · ${formatArtifactTime(artifactValue(artifact,'completedAt','finishedAt','updatedAt'))}</small></div></div><div class="template-preflight-artifact-grid">${checks.map(([label,key])=>{const passed=artifactCheck(artifact,key),state=passed===null?'pending':passed?'passed':'blocked',result=passed===null?(key==='trimContentParity'?'동일 Dataset 비교 대기':'검사 기록 없음'):passed?'통과':'확인 필요';return `<div class="${state}"><span>${label}</span><strong>${result}</strong></div>`}).join('')}<div class="${Number(artifactValue(artifact,'pageCount','pages'))>0?'passed':'blocked'}"><span>페이지 수</span><strong>${escape(artifactValue(artifact,'pageCount','pages')||'-')}면</strong></div><div class="passed"><span>파일 크기</span><strong>${formatArtifactBytes(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))}</strong></div></div>`}
