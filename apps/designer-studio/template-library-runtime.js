(()=>{
 ...
 async function openDeploy(record){
  ...
  const projectData=await projectForRecord(record),classification=projectData?.template?.classification,packageId=classification?.packageId||'',packageVersion=record.packageVersion||projectData?.template?.publishing?.packageVersion||'1.0.0';pendingDeploy={...record,classification};el('deployTemplateId').value=packageId;el('deployTemplateVersion').value=packageVersion;feedback.className=packageId?'save-feedback hidden':'save-feedback error';feedback.textContent=packageId?'':'저장된 시스템 베이스에 Package ID가 없습니다. 최종 분류를 다시 확인해 주세요.';button.disabled=!packageId;if(packageId)button.focus();
 }
