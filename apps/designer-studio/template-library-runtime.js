(()=>{
 ...
 function installTemplateSaveProgress(){
  ...
  window.ACDLTemplateSaveProgress=({phase='prepare',stage='',detail='',completed=0,total=1}={})=>{const publishRatios={version:8,prepare:18,upload:total?20+completed/total*50:20,validate:75,activate:86,cleanup:93},ratio=phase==='publishing'?(publishRatios[stage]||5):phase==='prepare'?5:phase==='assets'?(total?10+completed/total*55:65):phase==='publish'?(total?65+completed/total*25:88):phase==='version'?95:phase==='complete'?100:5,label=phase==='publishing'?`게시 단계 · ${detail||stage}`:phase==='prepare'?'저장할 템플릿을 준비하고 있습니다.':phase==='assets'?(total?`AI 이미지 ${completed}/${total} 저장 중`:'새로 저장할 이미지가 없습니다.'):phase==='publish'?`사용자 서비스 검토본 ${completed}/${total} 전송 중`:phase==='version'?'템플릿 버전을 저장하고 있습니다.':'저장이 완료되었습니다.';feedback.className='save-feedback info';feedback.innerHTML=`<div class="template-save-progress-copy"><span>${label}</span><strong>${Math.round(ratio)}%</strong></div><div class="template-save-progress-track"><div class="template-save-progress-bar" style="width:${ratio}%"></div></div>`};
 ...
 async function saveSettings(recordId,values){
  ...
  if(values.state==='published'){const published=await window.ACDLTemplatePublishing.publish({record,projectData,name:values.name||record.name,productType:record.type});values.name=published.name||values.name;projectData.template.metadata.name=values.name;saved={...saved,name:values.name}}
  ...
  renderLibrary(activeLibraryState);renderUserChoices();showEditorToast('템플릿 설정과 사용자 서비스 게시 목록을 동기화했습니다.');return saved
 }
