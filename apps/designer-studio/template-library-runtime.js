(()=>{
 ...
 function workerPowerShellCommand(artifact){const origin=String(artifact?.editorPrintOrigin||location.origin).replace(/\/$/,'').replace(/"/g,'`"'),jobId=String(artifact?.id||artifact?.jobId||'');return `$env:TEMPLATE_EDITOR_PRINT_ORIGIN="${origin}"\r\nnpm.cmd run pdf:worker -- --job "${jobId}" --once`}
 async function copyWorkerPowerShellCommand(artifact){const command=workerPowerShellCommand(artifact);try{await navigator.clipboard.writeText(command);showEditorToast('이 잡 전용 PowerShell 명령어를 복사했습니다.')}catch{const area=document.createElement('textarea');area.value=command;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();document.execCommand('copy');area.remove();showEditorToast('이 잡 전용 PowerShell 명령어를 복사했습니다.')}}
