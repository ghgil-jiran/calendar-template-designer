(()=>{
 ...
 function renderAiImageInspection(report){
  ...
  const excluded=inspection.disposition==='not_applicable',summary=inspection.criteriaSummary||{},recorded=criteria.filter(item=>item.source==='ai-image-print-quality'&&item.status!=='not_run').length,state=inspection.status==='passed'?'passed':inspection.status==='blocked'?'blocked':'pending',title=excluded?'AI 생성 이미지 없음 · 검사 제외':inspection.status==='passed'?'검사 완료':inspection.status==='blocked'?'개선 필요':recorded?'검사 진행 중 · 추가 확인 필요':'검사 준비 전',countText=`통과 ${summary.passed||0} · 확인 필요 ${summary.review||0} · 실패 ${summary.failed||0} · 검사 전 ${summary.notRun||0}`;
  const card=item=>{const cardState=item.status==='passed'?'passed':item.status==='failed'?'blocked':'pending',labels={passed:'통과',failed:'실패',review:'확인 필요',not_run:'검사 전'};return `<div class="${cardState}"><span>${item.source==='ai-image-print-quality'?'전용 검사':'기존 보고서 참고'}</span><strong>${labels[item.status]||'검사 전'}</strong><small>${escape(item.label||item.key)}</small>${item.message?`<small>${escape(item.message)}</small>`:''}</div>`};
  host.innerHTML=`<h3>AI 생성 이미지 검사</h3><div class="template-preflight-artifact-summary"><strong>${title}</strong><div><span>${excluded?'검사할 AI 생성 배경·일러스트가 없습니다.':`AI 생성 이미지 ${inspection.imageCount||0}개 · 필수 검사 ${summary.passed||0}/6 통과`}</span><small>${excluded?'4단계를 완료 처리하고 외부·실물 승인 단계로 진행할 수 있습니다.':inspection.criteriaComplete?'6개 전용 검사 근거가 모두 저장되었습니다.':recorded?`${countText} · 모든 항목이 통과해야 4단계를 완료합니다.`:'기존 품질 기록은 참고로 표시하며 4단계 합격 근거로 사용하지 않습니다.'}</small></div></div>${excluded?'':`<p class="template-preflight-artifact-wait">${recorded?'AI 생성 시 저장한 전용 검사 근거를 불러왔습니다.':'기존 AI 품질 보고서의 참고 근거만 연결되었습니다.'} 6개 검사가 모두 통과해야 4단계를 완료합니다.</p><h4>검사 기준</h4><div class="template-preflight-artifact-grid">${criteria.map(card).join('')}</div>`}${legacy?`<h4>기존 참고 기록</h4><div class="template-preflight-artifact-grid"><div class="pending"><span>기준 적용 전 기록</span><strong>합격 판정 제외</strong><small>${escape(legacy.message||'이전 이미지 배치 검토 기록')}</small></div></div>`:''}<h4>후속 범위</h4><div class="template-preflight-artifact-grid"><div class="pending"><span>최종 템플릿 저장</span><strong>추후 연결</strong><small>업스케일 처리 기준은 최종 템플릿 저장 단계에서 별도로 연결합니다.</small></div><div class="pending"><span>사용자 이미지</span><strong>사용자 서비스에서 검사</strong><small>실효 DPI·재업로드·저해상도 승인 여부는 사용자 서비스 범위입니다.</small></div></div>`;
 }
