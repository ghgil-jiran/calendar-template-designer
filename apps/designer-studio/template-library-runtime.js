(()=>{
 ...
 function renderPreflightArtifact(identity,artifact){
  ...
  const groups=[
   ...
   {scope:'follow-up',label:'후속 품질검토',checks:[['재단영역 내용 동등성','trimContentParity'],['템플릿 기본 이미지 승인','templateImageApproval']]},
   {scope:'runtime',label:'사용자 서비스 검사',checks:[['이미지 실배치 300DPI','imageDpi'],['최종 인쇄 이미지 승인','finalPrintImageApproval']]}
  ];
  ...
  const core=results.filter(item=>item.scope==='core'),coreFailed=core.filter(item=>item.result.status==='failed'),coreNotRun=core.filter(item=>!['passed','failed'].includes(item.result.status)),followUpOpen=results.filter(item=>item.scope==='follow-up'&&item.result.status!=='passed');
   const conditionalFollowUp=results.some(item=>item.scope==='follow-up'&&item.result.approvalLevel==='conditional'),summary=artifact.status==='queued'?'운영 PDF 워커 처리 대기':artifact.status==='processing'?'최종 PDF 생성·검사 중':coreFailed.length?'품질검사 실패':coreNotRun.length?'핵심 자동검사 확인 필요':followUpOpen.length?'1차 검사 완료 · 추가 확인 필요':conditionalFollowUp?'1차 검사 완료 · 잔여 과제 기록':'핵심 자동검사 통과';
   ...
   const trimReview=artifactCheck(artifact,'trimContentParity'),imageReview=artifactCheck(artifact,'templateImageApproval'),canRecordTrim=artifact.status==='done'&&trimReview.status!=='passed',canRecordImages=artifact.status==='done'&&imageReview.status!=='passed';
   host.innerHTML=`<h3>최종 PDF 자동검사</h3><div class="template-preflight-artifact-summary"><strong>${summary}</strong><div><span>${escape(artifactFilename(identity,artifact))}</span><small>잡 ${escape(artifact.id||artifact.jobId||'-')} · ${formatArtifactTime(artifactValue(artifact,'completedAt','finishedAt','updatedAt','createdAt'))}</small></div></div><p class="template-preflight-artifact-wait">핵심 자동검사 8개가 1차 완료 판정을 결정합니다. 후속 품질검토와 사용자 서비스 검사는 별도 범위입니다.</p>${['queued','processing'].includes(artifact.status)?'<p class="template-preflight-artifact-wait">워커 처리가 완료되면 아래 자동검사 결과가 갱신됩니다.</p>':''}${groups.map(group=>`<h4>${group.label}</h4><div class="template-preflight-artifact-grid">${results.filter(item=>item.scope===group.scope).map(card).join('')}</div>`).join('')}${canRecordTrim?'<div class="template-preflight-actions"><button id="recordTrimContentParityBtn" type="button">재단영역 수동 비교 결과 기록</button></div>':''}${canRecordImages?'<div class="template-preflight-actions"><button id="recordTemplateImageReviewBtn" type="button">기본 이미지 조건부 검토 결과 기록</button></div>':''}<div class="template-preflight-artifact-grid"><div class="${Number(artifactValue(artifact,'pageCount','pages'))>0?'passed':'pending'}"><span>페이지 수</span><strong>${escape(artifactValue(artifact,'pageCount','pages')||'-')}면</strong></div><div class="${Number(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))>0?'passed':'pending'}"><span>파일 크기</span><strong>${formatArtifactBytes(artifactValue(artifact,'fileSize','fileSizeBytes','byteLength','size'))}</strong></div></div>`;
   el('recordTrimContentParityBtn')?.addEventListener('click',recordTrimContentParityReview);
   el('recordTemplateImageReviewBtn')?.addEventListener('click',recordTemplateImageReview);
  }
 function renderPreflightHistory(history,identity,artifact){const host=el('templatePreflightHistory');if(!host)return;const items=Array.isArray(history)?history:Array.isArray(history?.jobs)?history.jobs:Array.isArray(history?.history)?history.history:artifact?[artifact]:[];host.innerHTML=`<h3>최근 산출물과 검사 이력</h3>${items.length?`<div class="template-preflight-history-list">${items.slice(0,8).map(item=>`<article><strong>${escape(artifactFilename(identity,item))}</strong><span>${escape(item?.report?.superseded?'완료본 재사용으로 종료':item.status||'-')}</span><span>${formatArtifactBytes(artifactValue(item,'fileSize','fileSizeBytes','byteLength','size'))}</span><small>${formatArtifactTime(artifactValue(item,'completedAt','finishedAt','updatedAt','createdAt'))}</small></article>`).join('')}</div>`:'<p>저장된 검사 이력이 없습니다.</p>'}`}
  ...
  async function recordTemplateImageReview(){
   ...
   if(!window.confirm('기본 이미지 검토를 조건부 완료로 기록할까요? 반복 이미지와 일부 흐린 표현은 잔여 과제로 유지됩니다.'))return;
   setPreflightBusy('recordTemplateImageReviewBtn',true,'기록 중…');
   try{const updated=await window.ACDLTemplatePublishing.recordTemplateImageReview(artifact.id,{pageCount,imagePageCount:14,verdict:'conditional',findings:['계절 샘플 이미지가 여러 달에 반복되어 최종 표준 템플릿에서 다양화 필요','일부 샘플 사진의 흐린 표현은 인쇄 선명도 관점에서 최종 교체 검토 필요']});preflightArtifact=normalizedArtifact(updated);await loadPreflightHistory();renderCurrentPreflight('템플릿 기본 이미지 검토를 조건부 완료로 기록했습니다.')}catch(error){showEditorToast(error?.message||'기본 이미지 검토 결과를 기록하지 못했습니다.');if(button)setPreflightBusy('recordTemplateImageReviewBtn',false,'기본 이미지 조건부 검토 결과 기록')}
  }
 ...
 function renderCurrentPreflight(message){
  ...
  ['runTemplateQuickPreflightBtn','runTemplateRenderCheckBtn','runTemplateFinalPreflightBtn','rerunTemplatePreflightBtn'].forEach((id,index)=>el(id)?.classList.toggle('hidden',preflightActiveGate!==Math.min(index,2)));if(preflightActiveGate===2)el('rerunTemplatePreflightBtn')?.classList.remove('hidden');el('templatePreflightArtifact')?.classList.toggle('hidden',preflightActiveGate!==2);setPreflightButtonLabel('runTemplateFinalPreflightBtn',preflightArtifact?.status==='done'?'검사 완료 · 다시 검사':preflightArtifact?.status==='processing'?'PDF 처리 중':preflightArtifact?.status==='queued'?'처리 대기 중':'최종 PDF 생성·검사');el('runTemplateFinalPreflightBtn').disabled=['queued','processing'].includes(preflightArtifact?.status);preflightRecord.preflightReport=report;
 }
...
async function runFinalPreflight(){
  ...
  try{const requestedArtifact=normalizedArtifact(await window.ACDLTemplatePublishing.ensurePrintPreflight(preflightIdentity,{force:true}));await loadPreflightHistory();preflightArtifact=latestPreflightArtifact(preflightHistory)||requestedArtifact;renderCurrentPreflight(preflightArtifact?.status==='done'?'완료된 최종 PDF 검사 결과를 불러왔습니다.':'최종 PDF 작업이 대기열에 등록됐습니다.')}catch(error){status.className='template-preflight-status blocked';status.innerHTML=`<strong>요청 실패</strong><span>${escape(error?.message||'최종 PDF 검사를 요청하지 못했습니다.')}</span>`}finally{setPreflightBusy('runTemplateFinalPreflightBtn',false);setPreflightButtonLabel('runTemplateFinalPreflightBtn',preflightArtifact?.status==='done'?'검사 완료 · 다시 검사':preflightArtifact?.status==='processing'?'PDF 처리 중':preflightArtifact?.status==='queued'?'처리 대기 중':'최종 PDF 생성·검사');if(['queued','processing'].includes(preflightArtifact?.status))el('runTemplateFinalPreflightBtn').disabled=true}
 }
