(()=>{
 ...
 async function openPrintPreflight(record){
  ...
  try{
   ...
   const labels={passed:['최종 인쇄 승인','자동검사·Acrobat 외부 Preflight·실물 인쇄 승인을 모두 완료했습니다.'],review:['인쇄 검증 진행 중',report.approval?.automated?.status==='passed'?'자동 인쇄 Preflight는 통과했습니다. Acrobat 외부 Preflight와 실물 인쇄 승인이 남아 있습니다.':report.printOutput?.contractReady&&!report.printOutput?.artifactVerified?(artifact?.status==='processing'?'운영 PDF 워커가 실제 CMYK PDF를 생성하고 있습니다. 완료 결과 새로고침으로 확인하세요.':artifact?.status==='queued'?'운영 PDF 워커 처리 대기열에 등록되어 있습니다. 새 작업은 만들지 않았습니다.':artifact?.status==='done'&&!report.printOutput?.artifactChecksComplete?`완료 PDF가 있지만 필수 세부검사가 남아 있습니다: ${(report.printOutput?.missingArtifactChecks||[]).join(', ')}`:'인쇄 계약은 준비됐지만 검증 완료된 실제 CMYK PDF가 필요합니다.'):`${report.summary.issueGroups}개 원인에서 ${report.summary.warnings}개 경고를 확인해 주세요.`],blocked:['수정 필요',`${report.summary.issueGroups}개 원인에서 ${report.summary.errors}개 오류가 발견됐습니다.`]},label=labels[report.status];status.className=`template-preflight-status ${report.status}`;status.innerHTML=`<strong>${label[0]}</strong><span>${label[1]}</span>`;renderPreflightStages(report);el('templatePreflightMetrics').innerHTML=[['원본 페이지',report.summary.pages],['Runtime 페이지',report.runtime?.pages||0],['화면 페이지',report.renderParity?.pages||0],['RGB PDF 페이지',report.renderParity?.pages||0],['오류',report.summary.errors],['경고',report.summary.warnings]].map(([name,value])=>`<div><strong>${value}</strong><span>${name}</span></div>`).join('');renderPreflightIssues();renderPreflightArtifact(identity,artifact);renderPreflightHistory(history,identity,artifact);
