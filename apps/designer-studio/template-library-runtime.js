(()=>{
 ...
 function renderPreflightArtifact(identity,artifact){
  ...
  const summary=artifact.status==='queued'?'운영 PDF 워커 처리 대기':artifact.status==='processing'?'최종 PDF 생성·검사 중':coreFailed.length?'품질검사 실패':coreNotRun.length?'핵심 자동검사 확인 필요':followUpOpen.length?'1차 검사 완료 · 추가 확인 필요':'핵심 자동검사 통과';
   const card=item=>{const {label,key,scope,result}=item,state=result.status==='passed'?'passed':scope==='core'&&result.status==='failed'?'blocked':'pending',manual=result?.evidence&&artifactValue(result,'evidence.sourceArtifact','evidence.targetArtifact'),title=scope==='runtime'?'사용자 서비스에서 검사':result.status==='passed'?(scope==='core'?'통과':manual?'통과 · 수동 비교':'완료'):scope==='follow-up'?'후속 확인':result.status==='failed'?'실패':'검사 전';return `<div class="${state}"><span>${label}</span><strong>${title}</strong>${result.message?`<small>${escape(result.message)}</small>`:''}${manual?`<small>${escape(artifactValue(result,'evidence.sourceArtifact'))} ↔ ${escape(artifactValue(result,'evidence.targetArtifact'))} · ${escape(formatArtifactTime(artifactValue(result,'reviewedAt')))}</small>`:''}</div>`};
