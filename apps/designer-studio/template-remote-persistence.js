(function(root){
 ...
 async function assetObjectUrls(ids,{concurrency=4}={}){
  const unique=[...new Set((ids||[]).filter(id=>/^[0-9a-f-]{36}$/i.test(id)))],urls={},failures=[];let next=0;
  const worker=async()=>{while(next<unique.length){const id=unique[next++];try{urls[id]=await assetObjectUrl(id)}catch(error){failures.push({id,error})}}};
  await Promise.all(Array.from({length:Math.min(Math.max(1,Number(concurrency)||1),unique.length||1)},worker));
 ...
 root.ACDLTemplateRemotePersistence=Object.freeze({isRemote,hasSession:()=>Boolean(accessToken()),accessToken,list,load,save,saveDraft,versions,hydrateVersion,restore,packagePreflight,remove,deletedCatalogKeys:()=>[...deletedCatalogKeys],toLibraryRecord:record,materializeAIDesignBackgrounds,aiDesignIntegrity,assertAIDesignIntegrity,prepareProjectData,hydrateProjectData,assetObjectUrl,assetObjectUrls,resolveAssetUrls,legacyStoragePath});
})(window);
