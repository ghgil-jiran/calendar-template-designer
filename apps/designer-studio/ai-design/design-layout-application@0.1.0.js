(function(root){
 ...
 function scaffold(role,page,project){
  const year=String(project.settings?.year||new Date().getFullYear()),startMonth=Number(project.settings?.startMonth||3),config=project.template?.settings?.aiDesignSpec?.dividerPages?.[page.id],defaultObjects=page.side==='back'?['title','school-building','school-logo','school-name','body']:['title','school-building','school-motto','school-song','school-tree','school-flower'],selected=new Set(config?.objects?.length?config.objects:defaultObjects),dividerCatalog={title:text('ai.divider.title','symbols-title',null,config?.purpose==='school-symbols'?'우리학교 상징':'우리 학교 이야기'),'school-name':text('ai.divider.school','school-name','school.name','학교명'),'school-building':semantic('ai.divider.building','school-building','school.profile.building','학교 전경'),'school-logo':semantic('ai.divider.logo','school-logo','school.profile.logo','교표'),'school-motto':semantic('ai.divider.motto','school-motto','school.profile.motto','교훈'),'school-song':semantic('ai.divider.song','school-song','school.profile.song','교가'),'school-tree':semantic('ai.divider.tree','school-tree','school.profile.tree','교목'),'school-flower':semantic('ai.divider.flower','school-flower','school.profile.flower','교화'),'image-slot':{id:'ai.divider.image',type:'image-frame',role:'user-image-slot',x:10,y:20,width:38,height:60,zIndex:2,image:{src:'',fit:'cover',binding:''},style:{}},'annual-calendar':{id:'ai.divider.annual',type:'year-calendar',role:'year-calendar',x:10,y:20,width:80,height:65,zIndex:2,startMonth,monthCount:12,columns:4,rowsMode:'inherit',style:{}},'mini-calendar':{id:'ai.divider.mini',type:'mini-calendar',role:'mini-calendar',x:55,y:20,width:35,height:32,zIndex:2,calendarYear:Number(year),calendarMonth:startMonth,style:{}},'schedule-list':{id:'ai.divider.schedule',type:'memo',role:'schedule-list',x:55,y:55,width:35,height:30,zIndex:2,title:'주요 일정',lineCount:8,style:{}},body:text('ai.divider.body','school-introduction',null,'학교 소개 내용을 입력하세요.')},dividerItems=Object.entries(dividerCatalog).filter(([id])=>selected.has(id)).map(([,item])=>item),items={
