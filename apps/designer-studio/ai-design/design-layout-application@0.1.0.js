(function(root){
 ...
 Object.entries(groups).forEach(([group,items])=>role==='divider'&&['symbols','secondary'].includes(group)?distributeGrid(items,profile[group]||profile.secondary):distribute(items,profile[group]||profile.primary||profile.support||profile.identity));
  ...
  if(role==='back-cover'){
   ...
   const identity=groups.identity||[],zone=backProfile.identity||[20,58,60,30],logo=identity.find(item=>item.role==='school-logo'),texts=identity.filter(item=>item!==logo),nameItems=texts.filter(item=>['school-name','school-english-name'].includes(item.role)),detailItems=texts.filter(item=>!nameItems.includes(item));
   if(logo)Object.assign(logo,{x:zone[0],y:zone[1],width:Math.min(18,zone[2]*.24),height:Math.min(16,Math.max(11,zone[3]*.42)),imageFit:'contain',cropAllowed:false,identityGroup:'school-identity'});
   const offset=logo?Math.min(21,zone[2]*.28):0,textX=zone[0]+offset,textWidth=zone[2]-offset,nameHeight=Math.min(15,Math.max(8,zone[3]*.38)),detailY=zone[1]+nameHeight+1,detailHeight=Math.max(3,zone[3]-nameHeight-1);
   distribute(nameItems,[textX,zone[1],textWidth,nameHeight]);distribute(detailItems,[textX,detailY,textWidth,detailHeight]);
   texts.forEach(item=>{item.style=item.style||{};const limits=item.role==='school-name'?[10,15]:item.role==='school-english-name'?[7,9]:[6.5,8];item.style.fontSize=Math.max(limits[0],Math.min(Number(item.style.fontSize)||limits[1],limits[1]));item.style.textAlign='left';item.identityGroup='school-identity'});
