(function(root){
 ...
 Object.entries(groups).forEach(([group,items])=>role==='divider'&&['symbols','secondary'].includes(group)?distributeGrid(items,profile[group]||profile.secondary):distribute(items,profile[group]||profile.primary||profile.support||profile.identity));
  ...
  if(role==='back-cover'){
   ...
   if(promotedLogo)Object.assign(promotedLogo,{imageFit:'contain',cropAllowed:false,identityGroup:'school-identity'});if(logo)Object.assign(logo,{x:zone[0],y:zone[1],width:Math.min(18,zone[2]*.24),height:Math.min(16,Math.max(11,zone[3]*.42)),imageFit:'contain',cropAllowed:false,identityGroup:'school-identity'});
