(function(root){
 ...
 function install(){
  const page=document.querySelector('[data-resource-content="ai-design"]');
  if(!page||document.getElementById('dividerCompositionCard'))return;
  const cards=[...page.querySelectorAll(':scope > .settings-card')],conditions=cards.find(card=>card.querySelector('h4')?.textContent.includes('디자인 생성 조건'))||cards.at(-1),card=document.createElement('div');
  ...
  const monthBack=conditions?.querySelector('.ai-month-back-options');
  if(monthBack)monthBack.after(card);else if(conditions)conditions.appendChild(card);else page.appendChild(card);
 ...
 function render(){
  ...
  const spec=current(),items=targets(),layouts=root.ACDLDesignTypeCatalog.roles.divider.options,saveButton=document.getElementById('saveDividerCompositionBtn');
 ...
 document.addEventListener('click',event=>{if(event.target.closest?.('[data-resource-page="ai-design"]'))setTimeout(()=>{install();render()},0)});
