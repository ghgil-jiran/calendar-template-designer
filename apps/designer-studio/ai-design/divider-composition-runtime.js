(function(root){
 ...
 const layoutsFor=purpose=>purpose==='school-symbols'?SCHOOL_SYMBOL_LAYOUTS:purpose==='school-introduction'?SCHOOL_INTRODUCTION_LAYOUTS:purpose==='yearly-plan'?YEARLY_PLAN_LAYOUTS:purpose==='academic-schedule'?ACADEMIC_SCHEDULE_LAYOUTS:GENERIC_LAYOUTS;
 ...
 function schoolIntroductionBoxes(objects,layoutId){const boxes=[],optional=objects.filter(id=>['school-logo','school-motto'].includes(id));if(layoutId==='school-intro-left-image'){boxes.push({id:'title',x:7,y:6,width:86,height:9},{id:'school-building',x:7,y:20,width:54,height:70});optional.forEach((id,index)=>boxes.push({id,x:66,y:20+index*28,width:27,height:id==='school-logo'?20:24}));return boxes}if(layoutId==='school-intro-background-image'){boxes.push({id:'school-building',x:5,y:5,width:90,height:88},{id:'title',x:10,y:9,width:80,height:10});optional.forEach((id,index)=>boxes.push({id,x:72,y:24+index*25,width:18,height:id==='school-logo'?16:21}));return boxes}boxes.push({id:'title',x:7,y:6,width:86,height:9},{id:'school-building',x:16,y:20,width:68,height:65});optional.forEach((id,index)=>boxes.push({id,x:16+index*24,y:87,width:22,height:8}));return boxes}
 ...
 function layoutCards(entry){const labels=Object.fromEntries(OBJECTS),boxes=(id)=>entry.purpose==='school-symbols'?symbolBoxes(entry.objects):entry.purpose==='school-introduction'?schoolIntroductionBoxes(entry.objects,id):entry.purpose==='yearly-plan'?yearlyPlanBoxes(id):entry.purpose==='academic-schedule'?scheduleBoxes(id):genericBoxes(entry.objects);return `<div class="divider-layout-options"><strong>3. 대표 배치</strong><div class="ai-layout-grid">${layoutsFor(entry.purpose).map(([id,name,description])=>`<label class="ai-layout-card${entry.layoutId===id?' selected':''}"><input type="radio" name="divider-layout-${esc(entry.pageId)}" value="${id}" data-divider-layout ${entry.layoutId===id?'checked':''}><header><strong>${name}</strong><span>${['academic-schedule','yearly-plan'].includes(entry.purpose)?'12개월':`${entry.objects.length}개`}</span></header><div class="ai-layout-wireframe symbol-layout-${id}">${boxes(id).map(box=>`<span class="ai-layout-box layout-${box.id}" style="left:${box.x}%;top:${box.y}%;width:${box.width}%;height:${box.height}%">${labels[box.id]||box.id}</span>`).join('')}</div>${description?`<small>${description}</small>`:''}</label>`).join('')}</div></div>`}
 function render(){
