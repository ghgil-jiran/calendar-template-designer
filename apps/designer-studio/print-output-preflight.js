((root)=>{
 ...
 function inspect(project,{artifact=null}={}){
  ...
  const checks=artifact?.checks||artifact?.preflight?.checks||{},requiredChecks={pdfx4:checks.pdfx4,outputIntent:checks.outputIntent,cmyk:checks.cmyk,k100:checks.k100,trimBox:checks.trimBox,bleedBox:checks.bleedBox,fontOutlined:checks.fontOutlined??checks.fontOutline,vectorContentPreserved:checks.vectorContentPreserved,trimContentParity:checks.trimContentParity,imageDpi:checks.imageDpi,templateImageApproval:checks.templateImageApproval,finalPrintImageApproval:checks.finalPrintImageApproval},checkStates=Object.fromEntries(Object.entries(requiredChecks).map(([key,value])=>[key,checkState(value)])),failedChecks=artifact?.status==='done'?Object.entries(checkStates).filter(([,state])=>state==='failed').map(([key])=>key):[],missingChecks=artifact?.status==='done'?Object.entries(checkStates).filter(([,state])=>state!=='passed'&&state!=='failed').map(([key])=>key):[];
  const artifactVerified=artifact?.verified===true&&failedChecks.length===0&&missingChecks.length===0;
  if(artifact?.status==='done'&&failedChecks.length)issues.push(issue('error','PRINT_ARTIFACT_CHECKS_FAILED',`최종 PDF의 필수 품질검사에 실패했습니다: ${failedChecks.join(', ')}`,'print.artifact.checks'));
  ...
  const approval=Object.freeze({automated:{status:automatedStatus,label:'자동 인쇄 Preflight'},external:{status:'not_run',label:'Acrobat 외부 Preflight',required:true},physical:{status:'not_run',label:'실물 인쇄 승인',required:true},finalApproved:false,artifactClass:generationMode.includes('native')?'native-print-model':artifact?'legacy-converted':'not-generated'});
  return Object.freeze({schemaVersion:'print-output-preflight.v2',approval,contractReady:Boolean(comparison?.contractReady)&&!issues.some(item=>item.severity==='error'&&item.code!=='PRINT_ARTIFACT_NOT_VERIFIED'&&item.code!=='PRINT_ARTIFACT_CHECKS_FAILED'),artifactVerified,artifactChecksComplete:failedChecks.length===0&&missingChecks.length===0,failedArtifactChecks:failedChecks,missingArtifactChecks:missingChecks,geometryMappingVerified:geometryValid,contentParity:{status:checkStates.trimContentParity==='not_run'?'same-dataset-required':checkStates.trimContentParity,comparisonBox:'TrimBox',message:'동일한 Runtime Dataset으로 생성한 Review PDF와 CMYK PDF의 TrimBox 영역을 비교해야 합니다.'},artifact:artifact||null,worker:'user-service-pdf-worker',profile:value,issues});
 }
