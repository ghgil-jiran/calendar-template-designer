...
- 학교 사용자가 달력을 만드는 서비스의 정식 명칭은 **학사달력 에디터 서비스**이며, **학사달력 사용자 서비스** 또는 **사용자 서비스**라고 부를 수 있다. 기존 코드명은 Calendar Workspace다.
- 관리자·디자이너가 템플릿을 만드는 도구의 정식 명칭은 **달력 템플릿 에디터**이며, **템플릿 에디터**라고 부를 수 있다. 기존 코드명은 Designer Studio다.
- 이후 확장성을 위해 제품 문서에서는 `학사달력 템플릿 에디터`라는 이름을 새 정식 명칭으로 사용하지 않는다.
...
1. **달력 템플릿 에디터(Designer Studio)**: 관리자·디자이너가 재사용 가능한 템플릿을 제작하고 검증한다.
2. **학사달력 에디터 서비스(Calendar Workspace)**: 학교 사용자가 템플릿을 선택하고 학교 데이터와 콘텐츠를 적용한다.
