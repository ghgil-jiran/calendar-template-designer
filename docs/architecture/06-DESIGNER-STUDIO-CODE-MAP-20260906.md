...
## 4차 구조 분리 준비 — Runtime 로드 계약

호환 Runtime의 함수 재할당을 소유 모듈로 통합하기 전에 현재 브라우저 실행 순서를 `features/runtime-load-manifest.json`으로 고정했다.

- 23개 Runtime마다 기존 source 식별자, 실제 파일, 소유 기능을 기록했다.
- 소유 기능은 Shell, 공통 Runtime, 개체 편집, 미리보기/PDF, 템플릿 설정의 5개 경계로 제한했다.
...
네 번째 통합에서는 연속 실행 관계가 만들어진 다음 묶음을 원문 순서 그대로 흡수했다.

- RC2 릴리스 메타데이터를 RC4·RC5 미리보기 Runtime에 포함
- 작업공간 설정, v35 보정, v36 데이터 플랫폼, v36.1 이미지 메모리·자동 복구, v36.2~v37.1 달력 유형 Runtime을 하나의 `template-settings-workspace-runtime.js`로 통합
- v36.3 안정화 코드는 실제 책임인 엽서형 모델·유형별 편집 설정으로 재분류

호환 Runtime manifest는 19개에서 12개로 줄었다. 결합된 7개 설정 블록과 미리보기 블록은 원본 커밋의 본문 순서와 일치하며 전체 빌드와 Studio 회귀검사 297개를 통과했다. 현재 편집 화면의 좌우 패널은 고정 영역이며 패널 자체 접기 기능은 검증 기준에 포함하지 않는다.

### 소유 기능별 후속 통합

- 미리보기 진입·페이지 이동·편집 복귀·전체 미리보기 닫기·Esc 처리를 `preview-entry-runtime.js`가 단독 소유하도록 Core의 중복 이벤트를 제거했다.
...
- `apps/designer-studio/index.html`: 80,812 bytes, 인라인 JavaScript 0개
- 기능 본체: AI 디자인, 월력 렌더링, 개체 편집, 미리보기/PDF, 템플릿 설정/라이브러리의 5개 소유 모듈
- 호환 실행 단계: 4개 소유 기능에 속한 10개 Runtime. 각 항목은 DOM 생성, Core 초기화, 라이브러리 초기화, 자동 맞춤처럼 서로 다른 실행 시점 경계를 보존한다.
- v32~v38, RC, Sprint 표기는 manifest의 원본 이력 추적 정보이며 독립 기능 소유권을 뜻하지 않는다.
- 전체 빌드, Studio 회귀검사 297개, Runtime 로드 순서 검사 통과를 구조 변경 완료 조건으로 삼는다.

따라서 실행 순서를 바꾸지 않고 추가로 합칠 수 있는 버전별 호환 블록은 없다. 이후 변경은 구조 정리가 아니라 개별 기능 개선으로 분리한다.
