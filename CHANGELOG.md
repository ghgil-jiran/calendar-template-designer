...
## 2026-09-05 · 템플릿 설정 화면 재구성

- 설정 흐름을 8단계로 정리하고 일정 등록을 독립 단계로 이동했으며 색상·폰트 테마를 통합했다.
- 기본 설정의 미니월력·공공 달력 선택과 사용자 입력 항목을 카드형 3열 UI로 통일했다.
- 학교 텍스트 정보, 연락처, 기본 샘플 이미지, 추가 이미지를 구분하고 역할별 이미지 등록·교체·삭제 팝업을 추가했다.
- 교훈은 텍스트, 교가는 이미지로 고정하고 `학교전경/표지`, `추가 이미지 등록` 명칭을 적용했다.
- OpenAI 연결은 상태를 우선 표시하고 `키 변경`을 눌렀을 때만 새 키 입력란을 열도록 변경했다.

## 2026-09-05 · 표지 배치안 적용

- 독립 프롬프트 규칙을 `school-calendar-prompt@0.2.1`로 올리고 빠른 시안(low)과 품질 시안(medium)을 선택하도록 했다.
- 네 가지 표지 구성마다 학교 사진·연도·학교 정보의 서로 다른 편집 개체 배치안을 반환한다.
- 선택한 AI 이미지는 배경 자산으로 두고 기존 표지 개체는 선택한 배치안에 맞춰 이동하므로 계속 편집할 수 있다.
