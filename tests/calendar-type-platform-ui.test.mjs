...
test('new calendar type opens one blank settings form without a clone menu',()=>{assert.match(runtime,/function blankType\(\)/);assert.match(runtime,/typePlatformAdd'\)\.onclick=\(\)=>detail\(blankType\(\),\{draft:true\}\)/);assert.match(runtime,/finishedSize:\{width:0,height:0/);assert.match(runtime,/name:''/);assert.doesNotMatch(runtime,/data-add-mode|현재 유형 복제|typeAddMenu/)});
test('calendar type UI uses the same control height and card hierarchy as template settings',()=>{const css=fs.readFileSync(new URL('../apps/designer-studio/calendar-type-platform.css',import.meta.url),'utf8');assert.match(css,/\.type-platform-shell button\{min-height:40px/);assert.match(css,/\.type-setting-card/);assert.match(css,/\.type-fields/);assert.match(css,/\.type-detail-footer/)});
