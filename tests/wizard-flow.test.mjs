...
test('published save transfers a snapshot review package before changing the library record', () => {
  const html=studioHtml;
  const runtime=fs.readFileSync(new URL('../apps/designer-studio/template-library-runtime.js',import.meta.url),'utf8');
  assert.match(html, /template-publishing-runtime\.js\?v=20260923\.1/);
...
test('remote template cards expose restore history without the unreliable preview action', () => {
  ...
  assert.match(studioHtml, /생성 준비 다시 검사/);
  assert.match(studioHtml, /최종 PDF 생성·검사/);
  assert.match(studioHtml, /id="templatePreflightCurrentTitle"/);
  assert.match(studioHtml, /class="template-preflight-completed"/);
  assert.ok(studioHtml.indexOf('id="templatePreflightStages"') < studioHtml.indexOf('class="template-preflight-current"'));
  assert.match(runtime, /refreshPreflightResult/);
  assert.match(runtime, /검사 완료 · 다시 검사/);
  assert.match(runtime, /data-preflight-gate/);
...
test('local studio server resolves extracted feature files from the Designer Studio directory', () => {
  ...
  assert.match(studioHtml, /features\/ai-design-runtime\.js\?v=20260923\.1/);
