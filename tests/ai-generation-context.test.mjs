...
test('an insert surface assigned to the yearly calendar is generated only as annual',()=>{const definition={id:'desk-standard',family:{id:'desk'},finishedSize:{width:260,height:180},productionSize:{width:266,height:186},policies:{annualSingle:'optional'}},value=project(definition,[{id:'surface.1.back',role:'front-insert-back',semanticPageRole:'yearly-calendar'}]),result=api().build(value),annual=result.roles.find(item=>item.role==='annual'),divider=result.roles.find(item=>item.role==='divider');assert.deepEqual([...annual.pageIds],['surface.1.back']);assert.deepEqual([...divider.pageIds],[]);assert.equal(result.pageTargets.filter(item=>item.pageId==='surface.1.back').length,1);assert.equal(result.pageTargets.find(item=>item.pageId==='surface.1.back').role,'annual')});
