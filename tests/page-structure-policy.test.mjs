...
test('basic settings expose a reviewable locked topology and optional annual selector',()=>{assert.match(source,/페이지 구성 정책/);assert.match(source,/연력은 선택 콘텐츠/);assert.match(source,/topology:'locked-standard'/);assert.match(source,/resourceAnnualPageMode/);assert.match(source,/resourceBackCoverFrontMode/)});
