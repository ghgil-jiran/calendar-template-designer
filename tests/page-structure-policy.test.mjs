...
test('first month back uses a separate surface by default',()=>{const value=project(),policy=api();delete value.template.settings.pageStructurePolicy;assert.equal(policy.DEFAULTS.firstMonthBackPlacement,'separate');assert.equal(policy.current(value).firstMonthBackPlacement,'separate')});
...
test('page settings expose one first-month-back placement selector with legacy compatibility',()=>{assert.match(source,/data-resource-content="page-settings"/);assert.match(source,/pageSettingsPolicyMount/);assert.match(source,/firstMonthBackPlacement/);assert.match(source,/topology:'surface-placement'/);assert.match(source,/coverBackMode:firstMonthBackPlacement==='cover-inside'/);assert.doesNotMatch(source,/resourceAnnualPageMode|resourceBackCoverFrontMode/)});

test('summary reports the actual rebuilt pages without projecting stale form counts',()=>{const value=project(),policy=api();value.settings={frontInsertCount:1,rearInsertCount:0};value.book.pageInstances.splice(2,0,{id:'insert-front',role:'front-insert-front'},{id:'insert-back',role:'front-insert-back'});const summary=policy.summary(value);assert.equal(summary.surfaceCount,8);assert.equal(summary.sheetCount,4);assert.equal(summary.dividerCount,2)});
