...
test('saved template preparation hydrates, migrates, validates and preserves all pages',async()=>{
 ...
 assert.equal(prepared.book.pageInstances.length,28);assert.deepEqual(calls,['hydrate','migrate','integrity']);assert.equal(prepared.template.resources.aiDesignAssets[0].src,'https://signed.example/bg.webp');
});
