...
assert.equal(sampleThree.template.documentVersion, 4);
...
assert.deepEqual(sampleThree.template.masters.calendar.calendarRegion, { x: 3.8, y: 2.8, width: 92.4, height: 92 });
...
assert.ok(sampleThree.template.masterElements['master.monthly.back'].filter(item => item.type === 'image-frame').every(item => /assets\/sample-three\/.+\.webp$/.test(item.image.src)));
...
assert.ok(sampleThree.template.masterElements['master.monthly.back'].filter(item => item.type.startsWith('mini-calendar')).every(item => item.showWeekdayHeader === true && item.monthLabelStyle === 'number-en'));
assert.ok(sampleThree.template.masterElements['master.monthly.back'].filter(item => item.type.startsWith('mini-calendar')).every(item => item.style.titleSize && item.style.sunday && item.style.saturday));
assert.deepEqual(sampleThree.template.masterElements['master.monthly.front'].map(item => item.role), ['school-motto-text', 'school-slogan', 'school-logo']);
assert.ok(sampleThree.book.monthlyStyleOverrides.every(item => item.tokens.plannerBackground === '#ffffff'));
assert.deepEqual(sampleThree.book.pageInstances.filter(item => item.role === 'monthly-back').map(item => item.photoLayout), ['triptych', 'triptych', 'hero', 'hero', 'triptych', 'hero', 'hero', 'triptych', 'triptych', 'hero', 'hero', 'hero']);
assert.equal(sampleThree.book.elementsByPage['surface.2.back'].filter(item => item.shadowOfMasterElementId?.startsWith('master.sample-3.photo.')).length, 3);
assert.equal(sampleThree.book.elementsByPage['surface.3.back'].filter(item => item.shadowOfMasterElementId?.startsWith('master.sample-3.photo.')).length, 3);
assert.ok(sampleThree.book.elementsByPage['surface.2.back'].filter(item => item.type === 'image-frame').every(item => /assets\/sample-three\/.+\.webp$/.test(item.image.src)));
assert.equal(sampleThree.book.elementsByPage['surface.4.back'].find(item => item.role === 'monthly-image').width, 53.5);
assert.equal(sampleThree.book.elementsByPage['surface.4.back'].find(item => item.role === 'monthly-image-secondary').opacity, 0);
...
const legacySampleThree = structuredClone(sampleThree);
legacySampleThree.template.documentVersion = 1;
legacySampleThree.template.masterElements['master.monthly.back'].forEach(item => { if (item.type === 'image-frame') delete item.image.src; });
legacySampleThree.book.pageInstances.filter(item => item.role === 'monthly-back').forEach(item => { delete item.photoLayout; legacySampleThree.book.elementsByPage[item.id] = []; });
const migratedSampleThree = globalThis.ACDLProjectDocument.migrateProject(legacySampleThree);
assert.deepEqual(migratedSampleThree.report.applied, ['desk-sample-3-photo-layouts-v2', 'desk-sample-3-document-version-2', 'desk-sample-3-calendar-details-v3', 'desk-sample-3-document-version-3', 'desk-sample-3-mini-calendar-styles-v4', 'desk-sample-3-fixed-surface-parity-v4', 'desk-sample-3-document-version-4']);
assert.equal(migratedSampleThree.project.template.documentVersion, 4);
assert.equal(migratedSampleThree.project.book.pageInstances.filter(item => item.photoLayout === 'hero').length, 7);
assert.deepEqual(globalThis.ACDLProjectDocument.migrateProject(migratedSampleThree.project).report.applied, []);
const versionThreeSample = structuredClone(sampleThree);
versionThreeSample.template.documentVersion = 3;
versionThreeSample.book.elementsByPage['surface.1.front'].find(item => item.id === 'page.sample-3.cover.students').image.src = 'data:image/png;base64,user-cover';
versionThreeSample.book.elementsByPage['surface.2.back'].find(item => item.type === 'image-frame').image.src = 'data:image/png;base64,user-month';
const migratedVersionThreeSample = globalThis.ACDLProjectDocument.migrateProject(versionThreeSample);
assert.equal(migratedVersionThreeSample.project.book.elementsByPage['surface.1.front'].find(item => item.id === 'page.sample-3.cover.students').image.src, 'data:image/png;base64,user-cover');
assert.equal(migratedVersionThreeSample.project.book.elementsByPage['surface.2.back'].find(item => item.type === 'image-frame').image.src, 'data:image/png;base64,user-month');
assert.deepEqual(migratedVersionThreeSample.report.applied, ['desk-sample-3-mini-calendar-styles-v4', 'desk-sample-3-fixed-surface-parity-v4', 'desk-sample-3-document-version-4']);
