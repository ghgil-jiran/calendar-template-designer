...
assert.equal(sampleThree.template.documentVersion, 3);
...
assert.deepEqual(sampleThree.template.masters.calendar.calendarLayout.regions, { titlePercent: 16, weekdayPercent: 5, dateGridPercent: 79 });
...
assert.ok(sampleThree.template.masterElements['master.monthly.back'].filter(item => item.type.startsWith('mini-calendar')).every(item => item.showWeekdayHeader === false && item.monthLabelStyle === 'number-en'));
...
assert.deepEqual(migratedSampleThree.report.applied, ['desk-sample-3-photo-layouts-v2', 'desk-sample-3-document-version-2', 'desk-sample-3-calendar-details-v3', 'desk-sample-3-document-version-3']);
assert.equal(migratedSampleThree.project.template.documentVersion, 3);
