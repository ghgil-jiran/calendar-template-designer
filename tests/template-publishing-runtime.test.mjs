...
test('first-page capture keeps source and clone styles aligned and ignores stale thumbnail hydration',()=>{const library=readFileSync(new URL('../apps/designer-studio/template-library-runtime.js',import.meta.url),'utf8');assert.ok(studioRuntimeSource.indexOf('const sourceNodes=[source,')<studioRuntimeSource.indexOf("clone.querySelectorAll('.editor-only"));assert.match(library,/const generation=\+\+thumbnailGeneration/);assert.match(library,/generation!==thumbnailGeneration\|\|!host\.isConnected/);assert.doesNotMatch(library,/library-thumbnail-fallback\" src=/)});
