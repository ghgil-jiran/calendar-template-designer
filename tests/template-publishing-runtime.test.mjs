...
test('every editor and lifecycle save refreshes the actual first page thumbnail',()=>{const library=readFileSync(new URL('../apps/designer-studio/template-library-runtime.js',import.meta.url),'utf8');assert.match(studioRuntimeSource,/cover=sourceProject\.book\.pageInstances\[0\]/);assert.match(studioRuntimeSource,/source:'first-page'/);assert.match(studioRuntimeSource,/ACDLRepresentativePreview\.refresh\(project/);assert.match(library,/ACDLRepresentativePreview\?\.refresh\?\.\(projectData/);assert.match(library,/result\.version\?\.projectData\?\.template\?\.thumbnail/)});
...
test('review proxy permits completed print artifact and AI image inspection modes',async()=>{for(const mode of ['print-preflight-download','print-preflight-history','record-ai-image-print-quality-review']){let forwarded=false;await forwardReviewPackage({authorization:'Bearer token',body:{mode,id:'job-id'},fetcher:async()=>{forwarded=true;return {ok:true,status:200,json:async()=>({ok:true})}}});assert.equal(forwarded,true)}});
...
test('the artifact summary distinguishes failed, unfinished, and fully passed checks',()=>{const library=readFileSync(new URL('../apps/designer-studio/template-library-runtime.js',import.meta.url),'utf8');assert.match(library,/coreFailed=core\.filter/);assert.match(library,/coreNotRun=core\.filter/);assert.match(library,/followUpOpen=results\.filter/);assert.match(library,/핵심 자동검사 실패/);assert.match(library,/핵심 자동검사 확인 필요/);assert.match(library,/핵심 자동검사 완료/)});
...
test('first-page capture keeps styles aligned and loads thumbnails within each grid',()=>{const library=readFileSync(new URL('../apps/designer-studio/template-library-runtime.js',import.meta.url),'utf8');assert.ok(studioRuntimeSource.indexOf('const sourceNodes=[source,')<studioRuntimeSource.indexOf("clone.querySelectorAll('.editor-only"));assert.match(library,/const thumbnailGenerations=new WeakMap\(\)/);assert.match(library,/container\.querySelector\(`/);assert.match(library,/hydrateThumbnails\(list,grid\)/);assert.match(library,/generation!==thumbnailGenerations\.get\(container\)/);assert.doesNotMatch(library,/library-thumbnail-fallback" src=/)});
