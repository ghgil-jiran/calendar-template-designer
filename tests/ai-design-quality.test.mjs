...
test('quality report checks every generated page and preserves print resolution as review',()=>{const {project,selected}=fixture(),report=api().createReport(project,selected,'2026-09-05T00:00:00.000Z');assert.equal(report.schemaVersion,'ai-design-quality.v1@0.2.0');assert.equal(report.status,'review-required');assert.equal(report.pageCount,3);assert.equal(report.failedPageCount,0);assert.equal(report.reviewPageCount,3);assert.ok(report.pages.every(page=>page.issues.some(issue=>issue.code==='print-resolution-review')))});
