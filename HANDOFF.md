...
- AI 생성 기반은 `school-calendar-design@0.4.0`과 대표 디자인 세트 2안이다. 표지·연력·학교 상징·월력·월력 뒷면·뒷표지 6개 역할 규칙 및 선택 세트의 나머지 11개월 확장 경로가 구현돼 있지만, 전체 페이지의 실제 생성 품질 검증은 완료되지 않았다.
- 템플릿 에디터 기준점은 `main@b7f213a`이며 전체 검사 230개와 인라인 검사 24개가 통과했다.
- `⑧ 디자인 유형 설정`은 UI 의견 수렴 단계다. 선택값을 설계 JSON·편집 개체·역할별 프롬프트에 실제 연결하는 일은 아래 순서로 진행한다.
...
- 이미 통과한 230개/24개 검사는 회귀 징후나 관련 코드 변경이 없으면 새 채팅 시작 시 반복하지 않는다.
