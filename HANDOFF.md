...
- 디자인 유형 1~7단계가 완료됐고 전체 검사 242개와 인라인 검사 24개가 통과했다. 현재는 선택 세트를 12개월·전체 페이지로 확장하는 8단계다.
...
- 8단계 `선택 세트를 12개월·전체 페이지로 확장`부터 계속한다.
...
- 이미 통과한 242개/24개 검사는 회귀 징후나 관련 코드 변경이 없으면 새 채팅 시작 시 반복하지 않는다.
