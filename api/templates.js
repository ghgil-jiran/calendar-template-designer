...
async function signedAsset(path){const result=await storage(`object/sign/template-packages/${path.split('/').map(encodeURIComponent).join('/')}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({expiresIn:3600})}),body=await result.json(),signed=body.signedURL||body.signedUrl;if(!signed)throw new Error('Signed asset URL missing');if(signed.startsWith('http'))return signed;const base=env('SUPABASE_URL').replace(/\/$/,'');return signed.startsWith('/storage/v1/')?`${base}${signed}`:`${base}/storage/v1${signed.startsWith('/')?'':'/'}${signed}`}
