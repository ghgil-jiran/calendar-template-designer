...
- 벽걸이형 `wall-academic-standard@0.1.0`을 시스템 베이스 `벽걸이형 표준 01 · 이미지 월력형`으로 연결했다.
- 편집기에서 Package를 열면 A3 세로 표지 1면과 2028.03~2029.02 월력 12면, 총 13면으로 재구성된다.
- 월별 이미지와 월력 frame, Package ID·버전, 편집 개체 위치는 Project 저장·재열기 후 유지된다.
- 자동검사: Studio 108/108, TypeScript 3개 패키지, 제품 상호작용, 인라인 스크립트 19개, 스타일 보호 통과.

## 다음 할 일

1. 사용자 로컬에서 시스템 베이스의 벽걸이형 표준 01을 연다.
2. 표지 1면과 3월~다음 해 2월 월력 12면, A3 세로 비율을 확인한다.
3. 월별 이미지 개체를 이동한 뒤 템플릿으로 저장하고 다시 열어 위치·13면·Package 버전이 유지되는지 확인한다.
4. 육안 확인 후 편집기 Project를 결정적 사용자 서비스 Template Package로 내보내는 경로를 완성한다.
5. 내보낸 Package를 사용자 서비스에서 선택·저장·재오픈·미리보기·PDF까지 검증한다.
