...
- 사용자 계정의 통합 개발 Supabase `calendar-editor-runtime-dev`에 템플릿 프로젝트·수정 불가 버전·자동저장 임시본·비공개 이미지 자산 저장 구조가 연결되어 있다.
- Vercel 환경에서 만든 내 템플릿은 Supabase에 영구 저장되며 다른 브라우저에서 최신 카드 한 개로 다시 열 수 있다.
- 명시적 저장은 새 버전을 만들고 자동저장은 버전을 만들지 않은 채 임시본 하나만 갱신한다.
- 내 템플릿 카드에는 최신 버전이 항상 표시되고 `버전 이력`을 펼치면 과거 저장본이 최신순으로 나타난다.
- 과거 버전은 비공개 이미지까지 포함해 읽기 전용으로 미리볼 수 있고, 복원하면 과거 기록을 변경하지 않고 새 최신 버전이 만들어진다.
- 미리보기 종료 버튼과 Esc는 과거 버전을 편집 상태로 노출하지 않고 템플릿 라이브러리로 복귀한다.
- 집중 검사 32건과 전체 Studio 검사 129건, 제품 상호작용, 인라인 스크립트 19개, Vercel 정적 출력, 스타일 보호 검사가 통과했다.
- 사용자 서비스의 기존 UI·저장·PDF 및 학사일정 AI 추출·PDF/X-4 보호 규칙은 변경하지 않았다.

## 다음 할 일

1. Vercel 화면에서 최신 버전 표시, 이력 펼치기, 과거 버전 읽기 전용 미리보기, 복원 후 새 버전 생성을 육안 확인한다.
2. 확인 결과와 달력 템플릿 에디터 지속 개선 항목을 사용자 서비스의 외부 대시보드에 기록한다.
3. 드래그 성능, Undo/Redo, 복제, 정렬, 잠금, 수치 입력의 첫 개선 묶음과 우선순위를 확정한다.
4. 개선된 편집기에서 벽걸이형 표준 01 디자인을 마무리하고 결정적 Template Package로 내보낸다.
5. 벽걸이형 Package를 사용자 서비스에서 선택·편집·저장·재오픈·13면 미리보기·PDF까지 검증한다.
