...
# 2026-09-13 · Runtime v2 페이지 의미 계약 1차 정규화

- 물리적인 면을 나타내는 `surfaceRole`과 그 면의 주용도를 나타내는 `contentPurpose`를 공통 계약에 분리했다.
- 연력과 학교 상징은 페이지 종류가 아니라 페이지 안의 콘텐츠 용도로 고정했다.
- 기존 `role`·`semanticPageRole`·`packageRole`은 과거 Package 호환용 별칭으로만 유지했다.
- 탁상형과 벽걸이형 Package 변환 결과가 새 의미 계약을 함께 내보내도록 보완했다.
- 계약 검사, Package 변환 검사, 전체 빌드 146건이 통과했다.
